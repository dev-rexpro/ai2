...
from rexpro_ai.internal.config import (
    STATE as _state,
)
from rexpro_ai.internal.config import (
    AppConfig,
...
from rexpro_ai.internal.config import (  # noqa: F401
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = AlembicConfig(OPEN_REXPRO_DIR / 'alembic.ini')

        migrations_path = OPEN_REXPRO_DIR / 'migrations'
...
STATIC_DIR = Path(os.getenv('STATIC_DIR', OPEN_REXPRO_DIR / 'static')).resolve()
...
if CUSTOM_NAME:
    try:
        r = requests.get(f'https://api.openrexpro.com/api/v1/custom/{CUSTOM_NAME}')
        data = r.json()
        if r.ok:
            if 'logo' in data:
                REXPRO_FAVICON_URL = url = (
                    f'https://api.openrexpro.com{data["logo"]}' if data['logo'][0] == '/' else data['logo']
                )
            ...
            if 'splash' in data:
                url = f'https://api.openrexpro.com{data["splash"]}' if data['splash'][0] == '/' else data['splash']
...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use all-in-one docker container (rexpro-ai + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = 'http://localhost:11434'
        else:
            OLLAMA_BASE_URL = 'http://host.docker.internal:11434'
    elif K8S_FLAG:
        OLLAMA_BASE_URL = 'http://ollama-service.rexpro-ai.svc.cluster.local:11434'
...
MILVUS_COLLECTION_PREFIX = os.getenv('MILVUS_COLLECTION_PREFIX', 'rexpro_ai')
...
QDRANT_COLLECTION_PREFIX = os.getenv('QDRANT_COLLECTION_PREFIX', 'rexpro-ai')
...
ELASTICSEARCH_INDEX_PREFIX = os.getenv('ELASTICSEARCH_INDEX_PREFIX', 'open_rexpro_collections')
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv('PINECONE_INDEX_NAME', 'rexpro-ai-index')
...
VALKEY_COLLECTION_PREFIX = os.getenv('VALKEY_COLLECTION_PREFIX', 'rexpro_ai')
...
REXPRO_URL = ConfigVar('REXPRO_URL', 'rexpro.url', os.getenv('REXPRO_URL', ''))


ENABLE_SIGNUP = ConfigVar(
    'ENABLE_SIGNUP',
    'ui.enable_signup',
    (False if not REXPRO_AUTH else os.getenv('ENABLE_SIGNUP', 'True').lower() == 'true'),
)
...
try:
    banners = json.loads(os.getenv('REXPRO_BANNERS', '[]'))
    banners = [BannerModel(**banner) for banner in banners]
except Exception as e:
    log.exception(f'Error loading REXPRO_BANNERS: {e}')
    banners = []

REXPRO_BANNERS = ConfigVar('REXPRO_BANNERS', 'ui.banners', banners)
...
Generate a concise, 3-5 word title summarizing the chat history. Do not include any emojis in the title.
### Guidelines:
...
if JWT_EXPIRES_IN.value == '-1':
    log.warning(
        ...
        '    See: https://docs.openrexpro.com/reference/env-configuration\n'
    )
