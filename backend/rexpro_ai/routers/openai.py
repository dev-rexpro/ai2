...
from rexpro_ai.utils.access_control import check_model_access, has_connection_access
from rexpro_ai.utils.anthropic import get_anthropic_models, is_anthropic_url
from rexpro_ai.utils.auth import get_admin_user, get_verified_user
from rexpro_ai.utils.headers import get_custom_headers, include_user_info_headers
from rexpro_ai.utils.misc import (
    convert_logit_bias_input_to_json,
    stream_chunks_handler,
)
from rexpro_ai.utils.payload import (
    apply_model_params_to_body_openai,
    apply_system_prompt_to_body,
)
from rexpro_ai.utils.session_pool import (
    cleanup_response,
    get_session,
    stream_wrapper,
)
...
async def get_headers_and_cookies(
    ...
    headers = {
        ...
        **(
            {
                'HTTP-Referer': 'https://openrexpro.com/',
                'X-Title': 'rexpro-ai',
            }
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'rexpro-ai: Server Connection Error',
            )
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    rexpro-ai stores extra bookkeeping fields (``id``, ``status``,
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='rexpro-ai: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
