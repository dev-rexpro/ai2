...
from rexpro_ai.models.users import (
    UpdateProfileForm,
    UserModel,
...
from rexpro_ai.utils.access_control import get_permissions, has_permission
...
from rexpro_ai.utils.groups import apply_default_group_assignment
from rexpro_ai.utils.misc import parse_duration, validate_email_format
from rexpro_ai.utils.oauth import auth_manager_config
from rexpro_ai.utils.rate_limit import RateLimiter
from rexpro_ai.utils.redis import get_redis_client
...
async def create_session_response(
    ...
    if set_cookie and response:
        ...
        response.set_cookie(
            ...
            httponly=True,
            samesite=REXPRO_AUTH_COOKIE_SAME_SITE,
            secure=REXPRO_AUTH_COOKIE_SECURE,
...
async def update_password(
    ...
    if REXPRO_AUTH_TRUSTED_EMAIL_HEADER:
...
async def signin(
    ...
    if REXPRO_AUTH_TRUSTED_EMAIL_HEADER:
        if REXPRO_AUTH_TRUSTED_EMAIL_HEADER not in request.headers:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=ERROR_MESSAGES.INVALID_TRUSTED_HEADER)

        email = request.headers[REXPRO_AUTH_TRUSTED_EMAIL_HEADER].lower()
        name = email

        if REXPRO_AUTH_TRUSTED_NAME_HEADER:
            name = request.headers.get(REXPRO_AUTH_TRUSTED_NAME_HEADER, email)
            try:
        ...
        user = await Auths.authenticate_user_by_email(email, db=db)
        if user:
            if REXPRO_AUTH_TRUSTED_GROUPS_HEADER:
                group_names = request.headers.get(REXPRO_AUTH_TRUSTED_GROUPS_HEADER, '').split(',')
                group_names = [name.strip() for name in group_names if name.strip()]

                if group_names:
                    await Groups.sync_groups_by_group_names(user.id, group_names, db=db)

            if REXPRO_AUTH_TRUSTED_ROLE_HEADER:
                trusted_role = request.headers.get(REXPRO_AUTH_TRUSTED_ROLE_HEADER, '').lower().strip()
...
async def signout(request: Request, response: Response, db: AsyncSession = Depends(get_async_session)):
                            ...
                            if logout_url:
                                return JSONResponse(
                                    ...
                                    content={
                                        ...
                                        + (
                                            f'&post_logout_redirect_uri={REXPRO_AUTH_SIGNOUT_REDIRECT_URL}'
                                            if REXPRO_AUTH_SIGNOUT_REDIRECT_URL
                                            else ''
                                        ),
                                    },
                                    headers=response.headers,
    ...
    if REXPRO_AUTH_SIGNOUT_REDIRECT_URL:
        return JSONResponse(
            ...
            content={
                'status': True,
                'redirect_url': REXPRO_AUTH_SIGNOUT_REDIRECT_URL,
            },
            headers=response.headers,
        )
...
async def get_admin_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        'ADMIN_EMAIL': request.app.state.config.ADMIN_EMAIL,
        'REXPRO_URL': request.app.state.config.REXPRO_URL,
...
async def update_admin_config(request: Request, form_data: AdminConfig, user=Depends(get_admin_user)):
    ...
    request.app.state.config.REXPRO_URL = form_data.REXPRO_URL
...
async def token_exchange(
    ...
    """
    Exchange an external OAuth provider token for an rexpro-ai JWT.
