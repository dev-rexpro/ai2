...
from rexpro_ai.config import BYPASS_ADMIN_ACCESS_CONTROL, STORAGE_LOCAL_CACHE, STORAGE_PROVIDER, UPLOAD_DIR
from rexpro_ai.constants import ERROR_MESSAGES
from rexpro_ai.internal.db import get_async_db_context, get_async_session
...
from rexpro_ai.models.files import (
    FileForm,
    FileListResponse,
    FileModel,
    FileModelResponse,
...
from rexpro_ai.routers.audio import transcribe
from rexpro_ai.routers.retrieval import ProcessFileForm, process_file
from rexpro_ai.storage.provider import Storage
from rexpro_ai.utils.auth import get_admin_user, get_verified_user
from rexpro_ai.utils.misc import strict_match_mime_type
...
async def upload_file_handler(
    ...
    try:
        ...
        contents, file_path = await asyncio.to_thread(
            ...
            filename,
            {
                'rexpro-ai-User-Email': user.email,
                'rexpro-ai-User-Id': user.id,
                'rexpro-ai-User-Name': user.name,
                'rexpro-ai-File-Id': id,
            },
        )
