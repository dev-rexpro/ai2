...
from rexpro_ai.constants import ERROR_MESSAGES, TASKS
from rexpro_ai.env import (
    ...
    REXPRO_ADMIN_EMAIL,
    REXPRO_ADMIN_NAME,
    REXPRO_ADMIN_PASSWORD,
    REXPRO_AUTH_SIGNOUT_REDIRECT_URL,
    REXPRO_AUTH_TRUSTED_EMAIL_HEADER,
    REXPRO_AUTH_TRUSTED_NAME_HEADER,
    REXPRO_BUILD_HASH,
    REXPRO_SECRET_KEY,
    REXPRO_SESSION_COOKIE_SAME_SITE,
    REXPRO_SESSION_COOKIE_SECURE,
)
from rexpro_ai.internal.db import ScopedSession, engine, get_async_session
from rexpro_ai.models.access_grants import AccessGrants
from rexpro_ai.models.channels import Channels
from rexpro_ai.models.chats import ChatForm, Chats
from rexpro_ai.models.functions import Functions
from rexpro_ai.models.messages import Messages
from rexpro_ai.models.models import Models
from rexpro_ai.models.users import UserModel, Users
from rexpro_ai.routers import (
    analytics,
    audio,
    auths,
    automations,
...
from rexpro_ai.routers.retrieval import (
    get_ef,
    get_embedding_function,
    get_reranking_function,
    get_rf,
)
from rexpro_ai.socket.main import (
    MODELS,
    get_event_emitter,
    get_models_in_use,
...
from rexpro_ai.utils import logger
from rexpro_ai.utils.actions import chat_action as chat_action_handler
from rexpro_ai.utils.asgi_middleware import (
    AuthTokenMiddleware,
    CommitSessionMiddleware,
    RedirectMiddleware,
    WebsocketUpgradeGuardMiddleware,
)
from rexpro_ai.utils.audit import AuditLevel, AuditLoggingMiddleware
from rexpro_ai.utils.auth import (
    create_admin_user,
    decode_token,
    get_admin_user,
...
from rexpro_ai.utils.chat import (
    chat_completed as chat_completed_handler,
)
from rexpro_ai.utils.chat import (
    generate_chat_completion as chat_completion_handler,
)
from rexpro_ai.utils.embeddings import generate_embeddings
from rexpro_ai.utils.logger import start_logger
from rexpro_ai.utils.middleware import (
    build_chat_response_context,
    process_chat_payload,
    process_chat_response,
)
from rexpro_ai.utils.models import (
    check_model_access,
    get_all_base_models,
    get_all_models,
    get_filtered_models,
)
from rexpro_ai.utils.oauth import (
...
from rexpro_ai.utils.plugin import install_tool_and_function_dependencies
from rexpro_ai.utils.redis import get_redis_client, get_redis_connection
from rexpro_ai.utils.security_headers import SecurityHeadersMiddleware
from rexpro_ai.utils.session_pool import get_session
from rexpro_ai.utils.tools import set_terminal_servers, set_tool_servers
...
async def lifespan(app: FastAPI):
    ...
    if REXPRO_ADMIN_EMAIL and REXPRO_ADMIN_PASSWORD:
        if await create_admin_user(REXPRO_ADMIN_EMAIL, REXPRO_ADMIN_PASSWORD, REXPRO_ADMIN_NAME):
    ...
    from rexpro_ai.utils.automations import scheduler_worker_loop
    ...
    from rexpro_ai.utils.session_pool import close_session
...
app.state.REXPRO_NAME = REXPRO_NAME
...
if ENABLE_OTEL:
    from rexpro_ai.utils.telemetry.setup import setup as setup_opentelemetry
...
app.state.config.REXPRO_URL = REXPRO_URL
...
app.state.config.BANNERS = REXPRO_BANNERS
...
from rexpro_ai.utils.access_control import has_permission, migrate_access_control
...
app.state.AUTH_TRUSTED_EMAIL_HEADER = REXPRO_AUTH_TRUSTED_EMAIL_HEADER
app.state.AUTH_TRUSTED_NAME_HEADER = REXPRO_AUTH_TRUSTED_NAME_HEADER
app.state.REXPRO_AUTH_SIGNOUT_REDIRECT_URL = REXPRO_AUTH_SIGNOUT_REDIRECT_URL
...
# `rexpro_ai.utils.asgi_middleware` for the rationale.
...
from rexpro_ai.utils.anthropic import (
...
async def generate_messages(
    ...
    All models configured in rexpro-ai are accessible via this endpoint.
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                'https://api.github.com/repos/rexpro-ai/rexpro-ai/releases/latest',
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for rexpro-ai.
...
try:
    if ENABLE_STAR_SESSIONS_MIDDLEWARE:
        ...
        app.add_middleware(
            ...
            cookie_same_site=REXPRO_SESSION_COOKIE_SAME_SITE,
            cookie_https_only=REXPRO_SESSION_COOKIE_SECURE,
        )
    ...
    app.add_middleware(
        SessionMiddleware,
        secret_key=REXPRO_SECRET_KEY,
        session_cookie='owui-session',
        same_site=REXPRO_SESSION_COOKIE_SAME_SITE,
        https_only=REXPRO_SESSION_COOKIE_SECURE,
    )
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            'name': app.state.REXPRO_NAME,
            'short_name': app.state.REXPRO_NAME,
            'description': f'{app.state.REXPRO_NAME} is an open, extensible, user-friendly interface for AI that adapts to your workflow.',
            'start_url': '/',
            'display': 'standalone',
...
async def get_opensearch_xml():
    ...
    <ShortName>{app.state.REXPRO_NAME}</ShortName>
    <Description>Search {app.state.REXPRO_NAME}</Description>
    <InputEncoding>UTF-8</InputEncoding>
    <Image width="16" height="16" type="image/x-icon">{app.state.config.REXPRO_URL}/static/favicon.png</Image>
    <Url type="text/html" method="get" template="{app.state.config.REXPRO_URL}/?q={'{searchTerms}'}"/>
    <moz:SearchForm>{app.state.config.REXPRO_URL}</moz:SearchForm>
