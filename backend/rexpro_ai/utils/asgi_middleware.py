...
from rexpro_ai.env import CUSTOM_API_KEY_HEADER
from rexpro_ai.internal.db import ScopedSession
from rexpro_ai.utils.auth import get_http_authorization_cred
...
class AuthTokenMiddleware:
    ...
    This is useful when rexpro-ai sits behind a reverse proxy that
    consumes the ``Authorization`` header for its own authentication —
    set the env var to a unique header (e.g. ``X-rexpro-ai-Key``) so
        ...
        if 'shared' in query_params and query_params['shared']:
            ...
            if text:
                ...
                if url_match:
                    ...
                    # YouTube URL.
                    from rexpro_ai.retrieval.loaders.youtube import _parse_video_id
