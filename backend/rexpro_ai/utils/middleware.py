...
from rexpro_ai.constants import TASKS
...
from rexpro_ai.models.folders import Folders
from rexpro_ai.models.functions import Functions
from rexpro_ai.models.models import Models
from rexpro_ai.models.oauth_sessions import OAuthSessions
from rexpro_ai.models.users import UserModel, Users
from rexpro_ai.retrieval.utils import get_sources_from_items
from rexpro_ai.routers.images import (
    CreateImageForm,
    EditImageForm,
    image_edits,
    image_generations,
)
from rexpro_ai.routers.memories import QueryMemoryForm, query_memory
from rexpro_ai.routers.pipelines import (
...
from rexpro_ai.routers.tasks import (
    generate_chat_tags,
    generate_follow_ups,
...
from rexpro_ai.utils.access_control import has_connection_access, has_permission
from rexpro_ai.utils.access_control.files import get_accessible_folder_files
from rexpro_ai.utils.chat import generate_chat_completion
from rexpro_ai.utils.code_interpreter import execute_code_jupyter
from rexpro_ai.utils.files import (
    convert_markdown_base64_images,
    get_file_url_from_base64,
    get_image_base64_from_url,
    get_image_url_from_base64,
)
from rexpro_ai.utils.filter import (
    get_sorted_filter_ids,
    process_filter_functions,
)

from rexpro_ai.utils.mcp.client import MCPClient
...
from rexpro_ai.utils.payload import apply_system_prompt_to_body
from rexpro_ai.utils.plugin import load_function_module_by_id
from rexpro_ai.utils.response import normalize_usage
from rexpro_ai.utils.sanitize import sanitize_code
from rexpro_ai.utils.task import (
    get_task_model_id,
    rag_template,
    tools_function_calling_generation_template,
)
from rexpro_ai.utils.tools import (
...
from rexpro_ai.utils.webhook import post_webhook
...
def serialize_output(output: list) -> str:
    ...
    for idx, item in enumerate(output):
        ...
        elif item_type == 'rexpro_ai:code_interpreter':
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    available_skills = []
    if all_skill_ids:
        from rexpro_ai.models.skills import Skills as SkillsModel
            ...
            if choices and choices[0].get('message', {}).get('content'):
                ...
                if content:
                    ...
                    if request.app.state.config.ENABLE_USER_WEBHOOKS and not await Users.is_user_active(user.id):
                        ...
                        if webhook_url:
                            await post_webhook(
                                request.app.state.REXPRO_NAME,
                                webhook_url,
                                f'{content}\n\n{title} - {request.app.state.config.REXPRO_URL}/c/{metadata["chat_id"]}',
                                {
                                    'action': 'chat',
                                    'message': content,
                                    'title': title,
                                    'url': f'{request.app.state.config.REXPRO_URL}/c/{metadata["chat_id"]}',
                                },
                            )
        ...
        async def response_handler(response, events):
            def tag_output_handler(content_type, tags, output):
                ...
                output_type_map = {
                    ...
                    'code_interpreter': 'rexpro_ai:code_interpreter',
                }
                ...
                if last_type == 'message':
                    ...
                    for start_tag, end_tag in tags:
                        ...
                        if match:
                            ...
                            elif output_item_type == 'rexpro_ai:code_interpreter':
                                output.append(
                                    {
                                        'type': 'rexpro_ai:code_interpreter',
                                        'id': output_id('ci'),
                                        'status': 'in_progress',
                                        'start_tag': start_tag,
                                        'end_tag': end_tag,
                    ...
                    or (last_type == 'rexpro_ai:code_interpreter' and content_type == 'code_interpreter')
                    ...
                    elif last_type == 'rexpro_ai:code_interpreter':
            ...
            try:
                                    ...
                                    if value:
                                        ...
                                        inside_tag_block = (
                                            ...
                                            and (
                                                ...
                                                or last_item_type == 'rexpro_ai:code_interpreter'
                                                or (
                                        ...
                                        if inside_tag_block:
                                            ...
                                            if last_item_type == 'rexpro_ai:code_interpreter':
                ...
                if DETECT_CODE_INTERPRETER:
                    ...
                    while output and output[-1].get('type') == 'rexpro_ai:code_interpreter' and retries < MAX_RETRIES:
                        await event_emitter(
                            {
                                'type': 'chat:completion',
                                'data': {
