...
SESSION_SECRET = REXPRO_SECRET_KEY
ALGORITHM = 'HS256'
...
def get_license_data(app, key):
    def data_handler(data):
        for k, v in data.items():
            if k == 'resources':
                ...
                setattr(app.state, 'REXPRO_NAME', v)
            elif k == 'metadata':
    ...
    if key:
        us = [
            'https://api.openrexpro.com',
            'https://licenses.api.openrexpro.com',
        ]
        try:
            for u in us:
                if handler(u):
        ...
        if data is not None and 'id' in data:
            ...
            if user is None:
                ...
                if REXPRO_AUTH_TRUSTED_EMAIL_HEADER:
                    trusted_email = request.headers.get(REXPRO_AUTH_TRUSTED_EMAIL_HEADER, '').lower()
