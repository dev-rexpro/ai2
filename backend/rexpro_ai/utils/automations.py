...
from rexpro_ai.internal.db import get_async_db
from rexpro_ai.models.automations import AutomationModel, AutomationRuns, Automations
from rexpro_ai.models.chats import ChatForm, Chats
from rexpro_ai.models.users import Users
from rexpro_ai.utils.task import prompt_template
...
async def _set_terminal_cwd(app, server_id: str, user, cwd: str, chat_id: str) -> None:
    ...
    Routes through the rexpro-ai terminal proxy endpoint so that
    auth headers, orchestrator policy routing, and X-User-Id are
    handled correctly — same path the frontend uses.
    """
    import aiohttp
    from rexpro_ai.env import AIOHTTP_CLIENT_SESSION_SSL
...
async def execute_automation(app, automation: AutomationModel) -> None:
    ...
    try:
        ...
        from rexpro_ai.socket.main import sio
...
async def _check_calendar_alerts(app) -> None:
    ...
    """
    from rexpro_ai.models.calendar import CalendarEvents, CalendarEventUpdateForm
    from rexpro_ai.socket.main import sio
    ...
    for event, user_tz in upcoming:
        ...
        try:
            rexpro_name = getattr(app.state, 'REXPRO_NAME', 'rexpro-ai')
