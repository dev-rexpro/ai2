...
# OPEN_REXPRO_DIR should be the directory where env.py resides (rexpro_ai/)
OPEN_REXPRO_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of OPEN_REXPRO_DIR (backend/)
BACKEND_DIR = OPEN_REXPRO_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (rexpro-ai-dev/)
...
if FROM_INIT_PY:
    PACKAGE_DATA = {'version': importlib.metadata.version('rexpro-ai')}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data('rexpro_ai', 'CHANGELOG.md') or b'').decode()
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv('DATA_DIR', OPEN_REXPRO_DIR / 'data')).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / 'open_rexpro_data', 'zip', DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv('DATA_DIR', OPEN_REXPRO_DIR / 'data'))

STATIC_DIR = Path(os.getenv('STATIC_DIR', OPEN_REXPRO_DIR / 'static'))

FONTS_DIR = Path(os.getenv('FONTS_DIR', OPEN_REXPRO_DIR / 'static' / 'fonts'))

FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', BASE_DIR / 'build')).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', OPEN_REXPRO_DIR / 'frontend')).resolve()
...
if os.path.exists(f'{DATA_DIR}/ollama.db'):
    # Rename the file
    os.rename(f'{DATA_DIR}/ollama.db', f'{DATA_DIR}/rexpro.db')
    log.info('Database migrated from Ollama-rexpro successfully.')
else:
    pass

DATABASE_URL = os.getenv('DATABASE_URL', f'sqlite:///{DATA_DIR}/rexpro.db')
...
if all(DB_VARS.values()):
    ...
    DATABASE_URL = f'sqlite+sqlcipher:///{DATA_DIR}/rexpro.db'
...
REDIS_KEY_PREFIX = os.getenv('REDIS_KEY_PREFIX', 'rexpro-ai')
...
REXPRO_AUTH = os.getenv('REXPRO_AUTH', 'True').lower() == 'true'
...
# REXPRO_JWT_SECRET_KEY is deprecated; use REXPRO_SECRET_KEY instead.
# No hardcoded fallback by design: the supported start scripts set/auto-generate it; unset is rejected below.
REXPRO_SECRET_KEY = os.getenv(
    'REXPRO_SECRET_KEY',
    os.getenv('REXPRO_JWT_SECRET_KEY', ''),
)

REXPRO_SESSION_COOKIE_SAME_SITE = os.getenv('REXPRO_SESSION_COOKIE_SAME_SITE', 'lax')
REXPRO_SESSION_COOKIE_SECURE = os.getenv('REXPRO_SESSION_COOKIE_SECURE', 'false').lower() == 'true'
REXPRO_AUTH_COOKIE_SAME_SITE = os.getenv('REXPRO_AUTH_COOKIE_SAME_SITE', REXPRO_SESSION_COOKIE_SAME_SITE)
REXPRO_AUTH_COOKIE_SECURE = (
    os.getenv(
        'REXPRO_AUTH_COOKIE_SECURE',
        os.getenv('REXPRO_SESSION_COOKIE_SECURE', 'false'),
    ).lower()
    == 'true'
)

if REXPRO_AUTH and REXPRO_SECRET_KEY == '':
    raise SystemExit(
        'REXPRO_SECRET_KEY is not set. It is a hard requirement when authentication is enabled.\n'
        'The supported start methods set or auto-generate it for you: use start.sh (Linux/macOS), '
        'start_windows.bat (Windows), or `rexpro-ai serve`.\n'
        'If you start the backend another way (e.g. invoking uvicorn directly, which is unsupported), '
        'you must set REXPRO_SECRET_KEY yourself to a long random value.\n'
        'See https://docs.openrexpro.com/reference/env-configuration#rexpro_secret_key'
    )
...
REXPRO_ADMIN_EMAIL = os.getenv('REXPRO_ADMIN_EMAIL', '')
REXPRO_ADMIN_PASSWORD = os.getenv('REXPRO_ADMIN_PASSWORD', '')
REXPRO_ADMIN_NAME = os.getenv('REXPRO_ADMIN_NAME', 'Admin')

REXPRO_AUTH_TRUSTED_EMAIL_HEADER = os.getenv('REXPRO_AUTH_TRUSTED_EMAIL_HEADER', None)
REXPRO_AUTH_TRUSTED_NAME_HEADER = os.getenv('REXPRO_AUTH_TRUSTED_NAME_HEADER', None)
REXPRO_AUTH_TRUSTED_GROUPS_HEADER = os.getenv('REXPRO_AUTH_TRUSTED_GROUPS_HEADER', None)
REXPRO_AUTH_TRUSTED_ROLE_HEADER = os.getenv('REXPRO_AUTH_TRUSTED_ROLE_HEADER', None)

# Custom header name for API key authentication.  Defaults to 'x-api-key'.
# Useful when rexpro-ai sits behind a reverse proxy / API gateway that
# already uses the Authorization header for its own authentication — set
# this to a unique header (e.g. 'X-rexpro-ai-Key') so the middleware
...
# compatible APIs for endpoints not natively handled by rexpro-ai.
ENABLE_OPENAI_API_PASSTHROUGH = os.getenv('ENABLE_OPENAI_API_PASSTHROUGH', 'False').lower() == 'true'

REXPRO_AUTH_SIGNOUT_REDIRECT_URL = os.getenv('REXPRO_AUTH_SIGNOUT_REDIRECT_URL', None)
...
OAUTH_CLIENT_INFO_ENCRYPTION_KEY = os.getenv('OAUTH_CLIENT_INFO_ENCRYPTION_KEY', REXPRO_SECRET_KEY)

OAUTH_SESSION_TOKEN_ENCRYPTION_KEY = os.getenv('OAUTH_SESSION_TOKEN_ENCRYPTION_KEY', REXPRO_SECRET_KEY)
...
# Allows external apps to exchange OAuth tokens for rexpro-ai tokens
...
REXPRO_NAME = os.getenv('REXPRO_NAME', 'rexpro-ai')
if REXPRO_NAME != 'rexpro-ai':
    REXPRO_NAME += ' (rexpro-ai)'

REXPRO_FAVICON_URL = 'https://rexpro-ai.rexpro.com/favicon.png'
REXPRO_BUILD_HASH = os.getenv('REXPRO_BUILD_HASH', 'dev-build')
...
FORWARD_USER_INFO_HEADER_USER_NAME = os.getenv('FORWARD_USER_INFO_HEADER_USER_NAME', 'X-rexpro-ai-User-Name')
FORWARD_USER_INFO_HEADER_USER_ID = os.getenv('FORWARD_USER_INFO_HEADER_USER_ID', 'X-rexpro-ai-User-Id')
FORWARD_USER_INFO_HEADER_USER_EMAIL = os.getenv('FORWARD_USER_INFO_HEADER_USER_EMAIL', 'X-rexpro-ai-User-Email')
FORWARD_USER_INFO_HEADER_USER_ROLE = os.getenv('FORWARD_USER_INFO_HEADER_USER_ROLE', 'X-rexpro-ai-User-Role')
FORWARD_SESSION_INFO_HEADER_MESSAGE_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_MESSAGE_ID', 'X-rexpro-ai-Message-Id')
FORWARD_SESSION_INFO_HEADER_CHAT_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_CHAT_ID', 'X-rexpro-ai-Chat-Id')

# If set while ENABLE_FORWARD_USER_INFO_HEADERS is True, send one signed HS256 JWT
# (FORWARD_USER_INFO_HEADER_JWT) instead of separate X-rexpro-ai-User-* headers.
FORWARD_USER_INFO_HEADER_JWT_SECRET = (os.environ.get('FORWARD_USER_INFO_HEADER_JWT_SECRET') or '').strip() or None
FORWARD_USER_INFO_HEADER_JWT = os.environ.get('FORWARD_USER_INFO_HEADER_JWT', 'X-rexpro-ai-User-Jwt')
try:
...
OTEL_SERVICE_NAME = os.getenv('OTEL_SERVICE_NAME', 'rexpro-ai')
