<script lang="ts">
	...
	const bulkToggleHandler = async (enable: boolean) => {
		...
		try {
			await Promise.all(
				targets.map((a) =>
					toggleAutomationById(localStorage.token, a.id),
				),
			);
		} catch (err) {
			toast.error(`${err}`);
...
<svelte:head>
	<title>{$i18n.t("Automations")} • {$REXPRO_NAME}</title>
</svelte:head>
						...
						<div class="gap-2 grid my-2 px-3">
							{#each automations as automation (automation.id)}
								...
								>
									...
									<div
										class="flex flex-row gap-0.5 self-center"
									>
										<AutomationMenu
											editHandler={() => {
												goto(
