<script lang="ts">
	...
	import { REXPRO_VERSION, REXPRO_API_BASE_URL } from "$lib/constants";
	...
	const setToolServers = async () => {
		let toolServersData = await getToolServersData(
		...
		const enabledTerminals = ($settings?.terminalServers ?? []).filter(
			(s) => s.enabled,
		);
	...
	onMount(async () => {
		...
		const isShortcutMatch = (event: KeyboardEvent, shortcut): boolean => {
			...
			const mainKeys = normalized.filter(
				(k) => !["ctrl", "shift", "alt", "mod"].includes(k),
			);
		...
		const setupKeyboardShortcuts = () => {
			document.addEventListener("keydown", async (event) => {
				if (isShortcutMatch(event, shortcuts[Shortcut.SEARCH])) {
					...
					isShortcutMatch(event, shortcuts[Shortcut.FOCUS_INPUT])
				) {
					...
					isShortcutMatch(
						event,
						shortcuts[Shortcut.COPY_LAST_CODE_BLOCK],
					)
				) {
					...
					isShortcutMatch(
						event,
						shortcuts[Shortcut.COPY_LAST_RESPONSE],
					)
				) {
					...
					isShortcutMatch(event, shortcuts[Shortcut.TOGGLE_SIDEBAR])
				) {
					console.log("Shortcut triggered: TOGGLE_SIDEBAR");
					event.preventDefault();
					showSidebar.set(!$showSidebar);
				} else if (
					isShortcutMatch(event, shortcuts[Shortcut.DELETE_CHAT])
				) {
					console.log("Shortcut triggered: DELETE_CHAT");
					event.preventDefault();
					document.getElementById("delete-chat-button")?.click();
				} else if (
					isShortcutMatch(event, shortcuts[Shortcut.OPEN_SETTINGS])
				) {
					console.log("Shortcut triggered: OPEN_SETTINGS");
					event.preventDefault();
					showSettings.set(!$showSettings);
				} else if (
					isShortcutMatch(event, shortcuts[Shortcut.SHOW_SHORTCUTS])
				) {
					console.log("Shortcut triggered: SHOW_SHORTCUTS");
					event.preventDefault();
					showShortcuts.set(!$showShortcuts);
				} else if (
					isShortcutMatch(event, shortcuts[Shortcut.CLOSE_MODAL])
				) {
					...
					isShortcutMatch(
						event,
						shortcuts[Shortcut.OPEN_MODEL_SELECTOR],
					)
				) {
					...
					isShortcutMatch(
						event,
						shortcuts[Shortcut.NEW_TEMPORARY_CHAT],
					)
				) {
					...
					isShortcutMatch(
						event,
						shortcuts[Shortcut.GENERATE_MESSAGE_PAIR],
					)
				) {
					console.log("Shortcut triggered: GENERATE_MESSAGE_PAIR");
					event.preventDefault();
					document
						.getElementById("generate-message-pair-button")
						?.click();
				} else if (
					isShortcutMatch(
						event,
						shortcuts[Shortcut.REGENERATE_RESPONSE],
					) &&
		...
		await showControls.set(
			!$mobile ? localStorage.showControls === "true" : false,
		);
			...
			{:else}
				{#if localDBChats.length > 0}
					<div class="fixed w-full h-full flex z-50">
						...
						>
							<div
								class="m-auto pb-44 flex flex-col justify-center"
							>
								<div class="max-w-md">
									<div
									...
									>
										...
										<span
											...
											>{$i18n.t("Settings")} > {$i18n.t(
												"Chats",
											)} > {$i18n.t("Import Chats")}</span
										>. {$i18n.t(
									...
									<div
										class=" mt-6 mx-auto relative group w-fit"
									>
										<button
