<script lang="ts">
	...
	import { REXPRO_NAME, mobile, showSidebar, user } from "$lib/stores";
	import {
		getCalendars,
		getCalendarEvents,
		deleteCalendar,
...
<svelte:head>
	<title>{$i18n.t("Calendar")} • {$REXPRO_NAME}</title>
</svelte:head>
...
>
	{#if loaded}
		...
		<nav
			class="px-3 pt-2 pb-2 backdrop-blur-xl drag-region select-none shrink-0"
		>
			<div class="flex items-center gap-1">
				...
				<div class="flex w-full items-center">
					<div class="flex items-center gap-0.5 py-1">
						<span class="min-w-fit px-1 text-sm select-none"
							>{headerText}</span
						>
						<button
		...
		<div class="flex flex-1 min-h-0">
			<!-- Sidebar -->
			<div
				class="hidden md:flex flex-col w-56 shrink-0 pr-1.5 pl-3 overflow-y-auto"
			>
				<CalendarSidebar
					{calendars}
					{visibleCalendarIds}
