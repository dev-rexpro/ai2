<script lang="ts">
	...
	import { settings, chatId, REXPRO_NAME, models, config } from "$lib/stores";
	...
	const loadSharedChat = async () => {
		...
		if (userSettings) {
			...
			let localStorageSettings = {} as Parameters<
				(typeof settings)["set"]
			>[0];
		...
		if (chat) {
			...
			if (chatContent) {
				...
				if (
					messages.length > 0 &&
					messages.at(-1)?.id &&
					messages.at(-1)?.id in history.messages
				) {
	...
	const cloneSharedChat = async () => {
		...
		const res = await cloneSharedChatById(
			localStorage.token,
			chat.id,
		).catch((error) => {
			toast.error(`${error}`);
...
<svelte:head>
	<title>
		{title
			? `${title.length > 30 ? `${title.slice(0, 30)}...` : title} • ${$REXPRO_NAME}`
			: `${$REXPRO_NAME}`}
	</title>
</svelte:head>
			...
			>
				...
				>
					<div class="px-3">
						...
						<div
							class="flex text-sm justify-between items-center mt-1"
						>
							<time
								class="text-gray-400"
								datetime={new Date(
									chat?.chat?.timestamp || Date.now(),
								).toISOString()}
							>
