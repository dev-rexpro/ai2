<script lang="ts">
	...
	import { REXPRO_NAME, config, user, socket } from "$lib/stores";
			...
			<div class="w-full px-10 min-h-screen flex flex-col text-center">
				{#if ($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false}
					<div class=" my-auto pb-10 w-full sm:max-w-md">
						...
						>
							<div>
								{$i18n.t("Signing in to {{REXPRO_NAME}}", {
									REXPRO_NAME: $REXPRO_NAME,
								})}
							</div>
				...
				{:else}
					<div
						class="my-auto flex flex-col justify-center items-center"
					>
						<div
								...
								<div class="mb-1">
									<div class=" text-2xl font-medium">
										{#if $config?.onboarding ?? false}
											{$i18n.t(
												`Get started with {{REXPRO_NAME}}`,
												{ REXPRO_NAME: $REXPRO_NAME },
											)}
										{:else if mode === "ldap"}
											{$i18n.t(
												`Sign in to {{REXPRO_NAME}} with LDAP`,
												{ REXPRO_NAME: $REXPRO_NAME },
											)}
										{:else if mode === "signin"}
											{$i18n.t(
												`Sign in to {{REXPRO_NAME}}`,
												{ REXPRO_NAME: $REXPRO_NAME },
											)}
										{:else}
											{$i18n.t(
												`Sign up to {{REXPRO_NAME}}`,
												{ REXPRO_NAME: $REXPRO_NAME },
											)}
										{/if}
									</div>
									...
									{#if $config?.onboarding ?? false}
										<div
											class="mt-1 text-xs font-medium text-gray-600 dark:text-gray-500"
										>
											ⓘ {$REXPRO_NAME}
											{$i18n.t(
						...
						{#if $config?.metadata?.login_footer}
							<div class="max-w-3xl mx-auto">
								<div
									class="mt-2 text-[0.7rem] text-gray-500 dark:text-gray-400 marked"
								>
									{@html DOMPurify.sanitize(
										marked($config?.metadata?.login_footer),
									)}
								</div>
							</div>
						{/if}
