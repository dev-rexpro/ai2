<script>
	...
	const setupSocket = async (enableWebsocket) => {
		const _socket = io(`${REXPRO_BASE_URL}` || undefined, {
			reconnection: true,
		...
		_socket.on("connect", async () => {
			...
			if (version !== null || deploymentId !== null) {
				if (
					($REXPRO_VERSION !== null && version !== $REXPRO_VERSION) ||
					($REXPRO_DEPLOYMENT_ID !== null &&
						deploymentId !== $REXPRO_DEPLOYMENT_ID)
				) {
					await unregisterServiceWorkers();
			...
			if (localStorage.getItem("token")) {
				...
				console.warn(
					"No token found in localStorage, user-join event not emitted",
				);
			}
		});
	...
	const executePythonAsWorker = async (id, code, cb, files = []) => {
		...
		let executing = true;
		let packages = [
			/\bimport\s+requests\b|\bfrom\s+requests\b/.test(code)
				? "requests"
				: null,
			/\bimport\s+bs4\b|\bfrom\s+bs4\b/.test(code)
				? "beautifulsoup4"
				: null,
			/\bimport\s+numpy\b|\bfrom\s+numpy\b/.test(code) ? "numpy" : null,
			/\bimport\s+pandas\b|\bfrom\s+pandas\b/.test(code)
				? "pandas"
				: null,
			/\bimport\s+matplotlib\b|\bfrom\s+matplotlib\b/.test(code)
				? "matplotlib"
				: null,
			/\bimport\s+seaborn\b|\bfrom\s+seaborn\b/.test(code)
				? "seaborn"
				: null,
			/\bimport\s+sklearn\b|\bfrom\s+sklearn\b/.test(code)
				? "scikit-learn"
				: null,
			...
			/\bimport\s+tiktoken\b|\bfrom\s+tiktoken\b/.test(code)
				? "tiktoken"
				: null,
	...
	const resolveToolServer = (serverUrl) => {
		let toolServer = $settings?.toolServers?.find(
	...
	const executeTool = async (data, cb, chatId) => {
		const { toolServer, toolServerData, token } = resolveToolServer(
			data.server?.url,
		);
		...
		if (toolServer) {
			...
			if (data?.name === "display_file" && data?.params?.path) {
				if (res?.exists !== false) {
					displayFileHandler(data.params.path, {
						showControls,
						showFileNavPath,
					});
				}
		...
		if (type === "calendar:alert" && data) {
			const timeStr =
				data.minutes_until <= 0
					...
					: data.minutes_until === 1
						...
						: $i18n.t("Starting in {{count}} minutes", {
								count: data.minutes_until,
							});
			...
			if ($isLastActiveTab) {
				if ($settings?.notificationEnabled ?? false) {
					new Notification(`${data.title} • rexpro-ai`, {
						body: timeStr,
						icon: `${REXPRO_BASE_URL}/static/favicon.png`,
					});
				}
			}
			return;
					...
					if (directConnections) {
						...
						try {
							if (API_CONFIG?.prefix_id) {
								...
								form_data["model"] = form_data["model"].replace(
									`${prefixId}.`,
									``,
								);
							}
		...
		if (
			...
			if (type === "chat:completion") {
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${displayTitle} • rexpro-ai`, {
								body: content,
								icon: `${REXPRO_BASE_URL}/static/favicon.png`,
							});
						}
	...
	const channelEventHandler = async (event) => {
		...
		// check url path
		const channel = $page.url.pathname.includes(
		...
		if ((!channel || isInBackground) && event?.user?.id !== $user?.id) {
			...
			if ($channels) {
				if (
					$channels.find((ch) => ch.id === event.channel_id) &&
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • rexpro-ai`, {
							body: data?.content,
							icon: `${REXPRO_API_BASE_URL}/users/${data?.user?.id}/profile/image`,
						});
					}
				}
	...
	const desktopEventHandler = async (event) => {
		...
		if (
			event.type === "query" &&
			(event.data?.query || event.data?.files?.length)
		) {
	...
	const windowMessageEventHandler = async (event) => {
		...
		if (
			event.data === "export:stats" ||
			event.data?.type === "export:stats"
		) {
	...
	onMount(async () => {
		...
		if (backendConfig) {
			...
			if ($config) {
				...
				if (localStorage.token && !backendOffline) {
					...
					if (sessionUser) {
						...
						try {
							...
							console.error(
								"Error refreshing backend config:",
								error,
							);
						}
...
<svelte:head>
	<title>{$REXPRO_NAME}</title>
	<link
		crossorigin="anonymous"
		rel="icon"
		href="{REXPRO_BASE_URL}/static/favicon.png"
	/>

	<meta name="apple-mobile-web-app-title" content={$REXPRO_NAME} />
	<meta name="description" content={$REXPRO_NAME} />
	<link
		rel="search"
...
{#if backendOffline}
	<div
		class="bg-red-500 text-white text-center py-2 text-xs font-semibold z-[9999] relative flex items-center justify-center gap-2"
	>
