	...
	>
		...
		<div class="overflow-y-auto scrollbar-thin max-h-60">
			...
			{#each filteredModels as model (model.id)}
				<button
					class="px-2.5 py-1.5 rounded-xl w-full text-left text-sm {model_id ===
					model.id
				...
				>
					<div
						class="flex text-black dark:text-gray-100 line-clamp-1"
					>
						<img
							src={`${REXPRO_API_BASE_URL}/models/model/profile/image?id=${encodeURIComponent(model.id)}`}
							alt={model?.name ?? model.id}
							class="rounded-md size-5 items-center mr-2"
							loading="lazy"
							on:error={(e) => {
