...
${content}
		...
		if (res) {
			...
			try {
				...
				if (jsonStartIndex !== -1 && jsonEndIndex !== -1) {
					const jsonResponse = response.substring(
						jsonStartIndex,
						jsonEndIndex + 1,
					);
	...
	const uploadFileHandler = async (file) => {
		...
		try {
			...
			if (uploadedFile) {
				...
				fileItem.file = await getFileById(
					localStorage.token,
					uploadedFile.id,
				).catch((e) => {
					toast.error(`${e}`);
	...
	const downloadHandler = async (type) => {
		...
		if (type === "txt") {
			const blob = new Blob([note.data.content.md], {
				type: "text/plain",
			});
	...
	const deleteNoteHandler = async (id) => {
		const res = await deleteNoteById(localStorage.token, id).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			},
							...
							if (line === "data: [DONE]") {
								...
								if (data.choices && data.choices.length > 0) {
									...
									if (choice.delta && choice.delta.content) {
										...
										enhancedContent.html = marked.parse(
											enhancedContent.md,
										);
...
<svelte:head>
	<title>
		{note?.title
			? `${note?.title.length > 30 ? `${note?.title.slice(0, 30)}...` : note?.title} • ${$REXPRO_NAME}`
			: `${$REXPRO_NAME}`}
	</title>
</svelte:head>
...
{#if note}
	<AccessControlModal
		...
		sharePublic={$user?.permissions?.sharing?.public_notes ||
			$user?.role === "admin"}
...
<PaneGroup direction="horizontal" class="w-full h-full">
	...
	>
		<div
			class="relative flex-1 w-full h-full flex justify-center pt-[11px]"
			id="note-editor"
		>
			{#if loading}
			...
			{:else}
				<div
					class=" w-full flex flex-col {loading ? 'opacity-20' : ''}"
				>
					<div
						class="shrink-0 w-full flex justify-between items-center px-3.5 mb-1.5"
					>
						<div class="w-full min-w-0 flex items-center">
							...
							<div
								class="flex items-center gap-0.5 shrink-0 translate-x-1"
							>
								{#if note?.write_access}
									{#if editor}
										<div>
					...
					<div class="  px-2.5">
						...
						>
							...
							>
								...
								{#if editor}
									<div
										class="flex items-center gap-1 px-1 min-w-fit"
									>
										<div>
											{$i18n.t("{{COUNT}} words", {
												COUNT: wordCount,
						...
						<RichTextInput
							...
							onFileDrop={(currentEditor, files, pos) => {
								files.forEach(async (file) => {
									const fileItem = await inputFileHandler(
										file,
									).catch((error) => {
										return null;
									});
