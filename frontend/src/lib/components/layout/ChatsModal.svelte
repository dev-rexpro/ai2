<script lang="ts">
	...
	const deleteHandler = async (chatId) => {
		const res = await deleteChatById(localStorage.token, chatId).catch(
			(error) => {
				toast.error(`${error}`);
			},
		);
			...
			{#if showSearch}
				<div class=" flex w-full space-x-2 mt-0.5 mb-1.5">
					<div class="flex flex-1">
						...
						{#if query}
							<div
								class="self-center pl-1.5 pr-1 translate-y-[0.5px] rounded-l-xl bg-transparent"
							>
								<button
			...
			>
				{#if chatList}
					<div class="w-full">
						...
						{/if}
						<div
							class="text-left text-sm w-full mb-3 max-h-[22rem] overflow-y-scroll"
						>
							{#if chatList.length === 0}
								<div
							...
							-->
								...
								>
									{#if showUserInfo && chat.user_id}
										<div
											class="w-32 shrink-0 flex items-center gap-2"
										>
											<img
												src="{REXPRO_API_BASE_URL}/users/{chat.user_id}/profile/image"
									...
									>
										<div
											class="text-ellipsis line-clamp-1 w-full"
										>
											{chat?.title}
										</div>
									</a>
									...
									<div
										class="{showUserInfo
											? 'w-28'
											: 'basis-2/5'} flex items-center justify-end"
									>
										<div
											class="hidden sm:flex text-gray-500 dark:text-gray-400 text-xs"
										>
											{$i18n.t(
												dayjs(
										...
										{#if !readOnly}
											<div
												class="flex justify-end pl-2.5 text-gray-600 dark:text-gray-300"
											>
												{#if unarchiveHandler}
													<Tooltip
														content={$i18n.t(
				...
				{:else}
					<div
						class="w-full h-full flex justify-center items-center min-h-20"
					>
