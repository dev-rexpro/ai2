<script lang="ts">
	...
	const deleteFeedbackHandler = async (feedbackId: string) => {
		const response = await deleteFeedbackById(
			localStorage.token,
			feedbackId,
		).catch((err) => {
			toast.error(err);
	...
	const exportHandler = async (format: "json" | "csv" = "json") => {
		const _feedbacks = await exportAllFeedbacks(
			localStorage.token,
			selectedModelId,
		).catch((err) => {
			toast.error(err);
...
{:else}
	...
	>
		...
		<div
			class="scrollbar-hidden relative whitespace-nowrap overflow-x-auto max-w-full px-2"
		>
			{#if (items ?? []).length === 0}
				<div
						...
						{#each items as feedback (feedback.id)}
							...
							>
								<td class=" py-0.5 text-right font-medium">
									<div class="flex justify-center">
										<Tooltip content={feedback?.user?.name}>
											<div class="shrink-0">
												<img
													src={`${REXPRO_API_BASE_URL}/users/${feedback.user.id}/profile/image`}
								...
								<td class=" py-1 pl-3 flex flex-col">
									<div
										class="flex flex-col items-start gap-0.5 h-full"
									>
										<div class="flex flex-col h-full">
											{#if feedback.data?.sibling_model_ids}
												...
												<Tooltip
													content={feedback.data.sibling_model_ids.join(
														", ",
													)}
												>
													<div
								...
								{#if feedback?.data?.rating}
									<td
										class="px-3 py-1 text-right font-medium text-gray-900 dark:text-white w-max"
									>
