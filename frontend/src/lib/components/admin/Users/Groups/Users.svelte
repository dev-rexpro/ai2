<script lang="ts">
	...
	const toggleMember = async (userId, state) => {
		if (state === "checked") {
			await addUserToGroup(localStorage.token, groupId, [userId]).catch(
				(error) => {
					toast.error(`${error}`);
					return null;
				},
			...
			await removeUserFromGroup(localStorage.token, groupId, [
				userId,
			]).catch((error) => {
				toast.error(`${error}`);
				return null;
		...
		{#if users.length > 0}
			...
			>
				...
				>
					...
					>
						<tr
							class=" border-b-[1.5px] border-gray-50/50 dark:border-gray-800/10"
						>
							<th
								scope="col"
