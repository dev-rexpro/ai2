<script lang="ts">
	...
	import { REXPRO_BUILD_HASH, REXPRO_VERSION } from "$lib/constants";
	...
	const checkForVersionUpdates = async () => {
		...
		version = await getVersionUpdates(localStorage.token).catch((error) => {
			return {
				current: REXPRO_VERSION,
				latest: REXPRO_VERSION,
			};
		});
	...
	const updateLdapServerHandler = async () => {
		...
		const res = await updateLdapServer(
			localStorage.token,
			LDAP_SERVER,
		).catch((error) => {
			toast.error(`${error}`);
		...
		{#if adminConfig !== null}
			<div class="">
				<div class="mb-3.5">
					...
					<hr
						class=" border-gray-100/30 dark:border-gray-850/30 my-2"
					/>

					<div class="mb-2.5">
						<div
							class=" mb-1 text-xs font-medium flex space-x-2 items-center"
						>
							<div>
								{$i18n.t("Version")}
							</div>
						...
						<div class="flex w-full justify-between items-center">
							...
							>
								<div class="flex gap-1">
									<Tooltip content={REXPRO_BUILD_HASH}>
										v{REXPRO_VERSION}
									</Tooltip>
						...
						<div class="flex w-full justify-between items-center">
							<div class="text-xs pr-2">
								...
								<div class=" text-xs text-gray-500">
									{$i18n.t(
										"Discover how to use rexpro-ai and seek support from the community.",
									)}
								</div>
							</div>
						...
						<div class="flex w-full justify-between items-center">
							<div class="text-xs pr-2">
								...
								{#if $config?.license_metadata}
									<a
										href="https://docs.openrexpro.com/enterprise"
										target="_blank"
										class="text-gray-500 mt-0.5"
									>
										<span
											class=" capitalize text-black dark:text-white"
										...
										<span
											...
											>{$config?.license_metadata
												?.seats ?? "Unlimited"} users.</span
										>
									</a>
									{#if $config?.license_metadata?.html}
										<div class="mt-0.5">
											{@html DOMPurify.sanitize(
												$config?.license_metadata?.html,
											)}
										</div>
									{/if}
								{:else}
				...
				<div class="mb-3">
					...
					<div
						class="mb-2.5 flex w-full items-center justify-between pr-2"
					>
						<div class=" self-center text-xs font-medium">
							{$i18n.t(
					...
					<div class="mb-2.5">
						...
						<Textarea
							...
							bind:value={
								adminConfig.PENDING_USER_OVERLAY_CONTENT
							}
						/>
					</div>
					...
					{#if adminConfig?.ENABLE_API_KEYS}
						<div class="mb-2.5 flex w-full justify-between pr-2">
							...
							<Switch
								bind:state={
									adminConfig.ENABLE_API_KEYS_ENDPOINT_RESTRICTIONS
								}
							/>
						</div>
						...
						{#if adminConfig?.ENABLE_API_KEYS_ENDPOINT_RESTRICTIONS}
							<div class=" flex w-full flex-col pr-2 mb-2.5">
								...
								<input
									...
									bind:value={
										adminConfig.API_KEYS_ALLOWED_ENDPOINTS
									}
								/>
								...
								>
									<a
										href="https://docs.openrexpro.com/reference/api-endpoints"
										target="_blank"
							...
							<div class="mt-2 text-xs">
								...
								>
									<div>
										...
										<span
											><a
												href="https://docs.openrexpro.com/reference/env-configuration#jwt_expires_in"
												target="_blank"
												class=" underline"
												>{$i18n.t(
							...
							{#if ENABLE_LDAP}
								<div class="flex flex-col gap-1">
									<div class="flex w-full gap-2">
										<div class="w-full">
											<div
												class=" self-center text-xs font-medium min-w-fit mb-1"
											>
												{$i18n.t("Label")}
											</div>
											<input
										...
										<div class="w-full">
											...
											>
												<input
													...
													bind:value={
														LDAP_SERVER.attribute_for_mail
													}
												/>
											</Tooltip>
										</div>
										...
										<div class="w-full">
											...
											>
												<input
													...
													bind:value={
														LDAP_SERVER.attribute_for_username
													}
												/>
											</Tooltip>
										</div>
									...
									<div>
										...
										{#if LDAP_SERVER.use_tls}
											<div class="flex w-full gap-2">
												<div class="w-full">
													<div
														class=" self-center text-xs font-medium min-w-fit mb-1 mt-1"
													>
														{$i18n.t(
															"Certificate Path",
														)}
						...
						<div class="mb-2.5 w-full justify-between">
							...
							<div class="flex mt-2 space-x-2">
								<input
									...
									bind:value={
										adminConfig.FOLDER_MAX_FILE_COUNT
									}
								/>
							</div>
					...
					<div class="mb-2.5 w-full justify-between">
						...
						<div class="flex mt-2 space-x-2">
							<input
								...
								bind:value={adminConfig.REXPRO_URL}
							/>
						</div>
						...
						>
							{$i18n.t(
								"Enter the public URL of your rexpro. This URL will be used to generate links in the notifications.",
							)}
						</div>
					</div>
