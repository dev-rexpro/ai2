<script lang="ts">
	...
	const updateOpenAIHandler = async () => {
		if (ENABLE_OPENAI_API !== null) {
			...
			OPENAI_API_BASE_URLS = OPENAI_API_BASE_URLS.map((url) =>
				url.replace(/\/$/, ""),
			);
			...
			if (OPENAI_API_KEYS.length !== OPENAI_API_BASE_URLS.length) {
				...
				if (OPENAI_API_KEYS.length > OPENAI_API_BASE_URLS.length) {
					OPENAI_API_KEYS = OPENAI_API_KEYS.slice(
						0,
						OPENAI_API_BASE_URLS.length,
					);
				}
				...
				if (OPENAI_API_KEYS.length < OPENAI_API_BASE_URLS.length) {
					const diff =
						OPENAI_API_BASE_URLS.length - OPENAI_API_KEYS.length;
	...
	const updateOllamaHandler = async () => {
		if (ENABLE_OLLAMA_API !== null) {
			// Remove trailing slashes
			OLLAMA_BASE_URLS = OLLAMA_BASE_URLS.map((url) =>
				url.replace(/\/$/, ""),
			);
	...
	const updateConnectionsHandler = async () => {
		const res = await setConnectionsConfig(
			localStorage.token,
			connectionsConfig,
		).catch((error) => {
			toast.error(`${error}`);
	...
	onMount(async () => {
		if ($user?.role === "admin") {
			...
			await Promise.all([
				...
				(async () => {
					connectionsConfig = await getConnectionsConfig(
						localStorage.token,
					);
				})(),
			]);
										...
										<OpenAIConnection
											...
											onDelete={() => {
												OPENAI_API_BASE_URLS =
													OPENAI_API_BASE_URLS.filter(
														(url, urlIdx) =>
															idx !== urlIdx,
													);
												OPENAI_API_KEYS =
													OPENAI_API_KEYS.filter(
														(key, keyIdx) =>
															idx !== keyIdx,
													);

												let newConfig = {};
												OPENAI_API_BASE_URLS.forEach(
													(url, newIdx) => {
														newConfig[newIdx] =
															OPENAI_API_CONFIGS[
																newIdx < idx
					...
					{#if ENABLE_OLLAMA_API}
						<div class="">
							...
							<div class="flex w-full gap-1.5">
								<div
									class="flex-1 flex flex-col gap-1.5 mt-1.5"
								>
									{#each OLLAMA_BASE_URLS as url, idx}
										<OllamaConnection
											bind:url={OLLAMA_BASE_URLS[idx]}
											bind:config={
												OLLAMA_API_CONFIGS[idx]
											}
											{idx}
											onSubmit={() => {
												updateOllamaHandler();
											...
											onDelete={() => {
												OLLAMA_BASE_URLS =
													OLLAMA_BASE_URLS.filter(
														(url, urlIdx) =>
															idx !== urlIdx,
													);

												let newConfig = {};
												OLLAMA_BASE_URLS.forEach(
													(url, newIdx) => {
														newConfig[newIdx] =
															OLLAMA_API_CONFIGS[
																newIdx < idx
							...
							>
								...
								<a
									...
									href="https://github.com/dev-rexpro/rexpro-ai#troubleshooting"
									target="_blank"
								>
					...
					<div class="flex justify-between items-center text-sm">
						...
						<div class="flex items-center">
							<div class="">
								<Switch
									bind:state={
										connectionsConfig.ENABLE_DIRECT_CONNECTIONS
									}
									on:change={async () => {
					...
					<div class="flex justify-between items-center text-sm">
						...
						<div class="flex items-center">
							<div class="">
								<Switch
									bind:state={
										connectionsConfig.ENABLE_BASE_MODELS_CACHE
									}
									on:change={async () => {
										updateConnectionsHandler();
									}}
