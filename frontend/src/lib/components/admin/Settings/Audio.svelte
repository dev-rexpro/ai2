<script lang="ts">
	...
	const updateConfigHandler = async () => {
		let openaiParams = {};
		try {
			openaiParams = TTS_OPENAI_PARAMS
				? JSON.parse(TTS_OPENAI_PARAMS)
				: {};
	...
	onMount(async () => {
		...
		if (res) {
			...
			STT_SUPPORTED_CONTENT_TYPES = (
				res?.stt?.SUPPORTED_CONTENT_TYPES ?? []
			).join(",");
			...
			STT_MISTRAL_USE_CHAT_COMPLETIONS =
				res.stt.MISTRAL_USE_CHAT_COMPLETIONS;
		}
			...
			<div>
					...
					<div class="flex items-center relative">
						...
						>
							...
							<option value="deepgram"
								>{$i18n.t("Deepgram")}</option
							>
							<option value="azure"
								>{$i18n.t("Azure AI Speech")}</option
							>
							<option value="mistral"
								>{$i18n.t("MistralAI")}</option
							>
						</select>
					</div>
				</div>
				...
				{:else if STT_ENGINE === ""}
					<div>
						...
						>
							{$i18n.t(
								`rexpro-ai uses faster-whisper internally.`,
							)}
					...
					<div class="flex items-center relative">
						...
						>
							...
							<option value="transformers"
								>{$i18n.t("Transformers")} ({$i18n.t(
									"Local",
								)})</option
							>
							<option value="openai">{$i18n.t("OpenAI")}</option>
							<option value="elevenlabs"
								>{$i18n.t("ElevenLabs")}</option
							>
							<option value="azure"
								>{$i18n.t("Azure AI Speech")}</option
							>
				...
				<div class="mb-2">
					...
					{:else if TTS_ENGINE === "transformers"}
						<div>
							...
							>
								{$i18n.t(
									`rexpro-ai uses SpeechT5 and CMU Arctic speaker embeddings.`,
								)}
