<script lang="ts">
	...
	const pullModelHandler = async () => {
		...
		if (
			Object.keys($MODEL_DOWNLOAD_POOL).length === MAX_PARALLEL_DOWNLOADS
		) {
			toast.error(
				$i18n.t(
	...
	const uploadModelHandler = async () => {
		...
		if (modelUploadMode === "file") {
			...
			if (file) {
				...
				fileResponse = await uploadModel(
					localStorage.token,
					file,
					urlIdx,
				).catch((error) => {
	...
	const deleteModelHandler = async () => {
		...
		if (res) {
			toast.success(
				$i18n.t(`Deleted {{deleteModelTag}}`, { deleteModelTag }),
			);
		}
		...
		ollamaModels = await getOllamaModels(localStorage.token, urlIdx).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			},
	...
	<div class=" flex flex-col w-full">
		<div>
			<div class="space-y-2">
				<div>
					<div
						class=" mb-2 text-sm font-medium flex items-center gap-1.5"
					>
						<div>
					...
					<div
						class="mt-2 mb-1 text-xs text-gray-400 dark:text-gray-500"
					>
						{$i18n.t(
					...
					{#if updateModelId}
						<div class="text-xs flex justify-between items-center">
							<div>
								Updating "{updateModelId}" {updateProgress
									? `(${updateProgress}%)`
									: ""}
							</div>
						...
						{#each Object.keys($MODEL_DOWNLOAD_POOL) as model}
							{#if "pullProgress" in $MODEL_DOWNLOAD_POOL[model]}
								<div class="flex flex-col">
									...
									<div class="">
										<div
											class="flex flex-row justify-between space-x-4 pr-2"
										>
											<div class=" flex-1">
												<div
					...
					>
							...
							>
								{#if modelUploadMode === "file"}
									<span class="ml-2 self-center"
										>{$i18n.t("File Mode")}</span
									>
								{:else}
									<span class="ml-2 self-center"
										>{$i18n.t("URL Mode")}</span
									>
								{/if}
							</button>
						</div>
						...
						<div class="flex w-full mb-1.5">
							<div class="flex flex-col w-full">
								{#if modelUploadMode === "file"}
									<div
										class="flex-1 {modelInputFile &&
										modelInputFile.length > 0
											? 'mr-2'
											: ''}"
									>
										<input
						...
						{#if uploadMessage}
							<div class="mt-2">
								...
								<div
									class="w-full rounded-full dark:bg-gray-800"
								>
									<div
										class="dark:bg-gray-600 bg-gray-500 text-xs font-medium text-gray-100 text-center p-0.5 leading-none rounded-full"
										style="width: 100%"
