<script>
	...
	let profileImageUrl = `${REXPRO_BASE_URL}/favicon.png`;
	let description = "";
	...
	const submitHandler = () => {
		...
		if (!name || !id) {
			loading = false;
			toast.error(
				$i18n.t("Name and ID are required, please fill them out"),
			);
			return;
		}
		...
		name = "";
		id = "";
		profileImageUrl = `${REXPRO_BASE_URL}/favicon.png`;
		description = "";
		modelIds = [];
		selectedModelId = "";
	};
			...
			>
				...
				>
					<div class="px-1">
						...
						<hr
							class=" border-gray-100 dark:border-gray-700/10 my-2.5 w-full"
						/>
							...
							{#if modelIds.length > 0}
								<div class="flex flex-col">
									{#each modelIds as modelId, modelIdx}
										...
										>
											<div
												class=" text-sm flex-1 py-1 rounded-lg"
											>
												{$models.find(
													(model) =>
														model.id === modelId,
					...
					<div
						class="flex justify-end pt-3 text-sm font-medium gap-1.5"
					>
						{#if edit}
							<button
