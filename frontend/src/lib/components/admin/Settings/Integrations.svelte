<script lang="ts">
	...
	const saveTerminalServers = async () => {
		...
		if (res) {
			...
			const existingDirectTerminals = ($terminalServers ?? []).filter(
				(t) => !t.id,
			);
			const systemTerminals = await getTerminalServers(
				localStorage.token,
			);
...
<AddTerminalServerModal
	...
	connection={editTerminalIdx !== null
		? terminalConnections[editTerminalIdx]
		: null}
	onSubmit={(c) => {
		...
		{#if servers !== null}
			<div class="">
				<div class="mb-3">
					...
					<hr
						class=" border-gray-100/30 dark:border-gray-850/30 my-4"
					/>
					...
					<div class="mb-2.5 flex flex-col w-full">
									...
									>
										<div class="flex w-full">
											...
											>
												...
												<div
													class="outline-hidden w-full bg-transparent text-sm"
												>
													{connection.name ||
														connection.url ||
														$i18n.t("New Terminal")}
						...
						<div class="mt-1.5">
							...
							>
								<a
									class="underline"
									href="https://github.com/rexpro-ai/open-terminal"
									target="_blank"
