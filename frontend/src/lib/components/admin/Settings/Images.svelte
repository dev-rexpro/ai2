<script lang="ts">
	...
	const getModels = async () => {
		models = await getImageGenerationModels(localStorage.token).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			},
	...
	const saveHandler = async () => {
		...
		if (config?.COMFYUI_WORKFLOW) {
			if (!validateJSON(config?.COMFYUI_WORKFLOW)) {
				toast.error(
					$i18n.t("Invalid JSON format for ComfyUI Workflow."),
				);
				loading = false;
				return;
			}

			config.COMFYUI_WORKFLOW_NODES = REQUIRED_WORKFLOW_NODES.map(
				(node) => {
					return {
						type: node.type,
						key: node.key,
		...
		if (config?.IMAGES_EDIT_COMFYUI_WORKFLOW) {
			if (!validateJSON(config?.IMAGES_EDIT_COMFYUI_WORKFLOW)) {
				toast.error(
					$i18n.t("Invalid JSON format for ComfyUI Edit Workflow."),
				);
				loading = false;
				return;
			}
	...
	onMount(async () => {
		if ($user?.role === "admin") {
			...
			REQUIRED_WORKFLOW_NODES = REQUIRED_WORKFLOW_NODES.map((node) => {
				const n =
					config.COMFYUI_WORKFLOW_NODES.find(
						(n) => n.type === node.type,
					) ?? node;
				console.debug(n);
			...
			REQUIRED_EDIT_WORKFLOW_NODES = REQUIRED_EDIT_WORKFLOW_NODES.map(
				(node) => {
					const n =
						config.IMAGES_EDIT_COMFYUI_WORKFLOW_NODES.find(
							(n) => n.type === node.type,
						) ?? node;
					console.debug(n);
				...
				<div class="mb-3">
					...
					{#if config.ENABLE_IMAGE_GENERATION}
						...
						<div class="mb-2.5">
							...
							>
								...
								<Switch
									bind:state={
										config.ENABLE_IMAGE_PROMPT_GENERATION
									}
								/>
							</div>
						</div>
					...
					<div class="mb-2.5">
						<div class="flex w-full justify-between items-center">
							...
							>
								<option value="openai"
									>{$i18n.t("Default (Open AI)")}</option
								>
								<option value="comfyui"
									>{$i18n.t("ComfyUI")}</option
								>
								<option value="automatic1111"
									>{$i18n.t("Automatic1111")}</option
								>
								<option value="gemini"
									>{$i18n.t("Gemini")}</option
								>
							</select>
						</div>
					</div>
					...
					{:else if (config?.IMAGE_GENERATION_ENGINE ?? "automatic1111") === "automatic1111"}
						<div class="mb-2.5">
							...
							>
								{$i18n.t(
									"Include `--api` flag when running stable-diffusion-rexpro",
								)}
								<a
									class=" text-gray-300 font-medium"
									href="https://github.com/AUTOMATIC1111/stable-diffusion-rexpro/discussions/3734"
									target="_blank"
								>
						...
						<div class="mb-2.5">
							...
							>
								{$i18n.t(
									"Include `--api-auth` flag when running stable-diffusion-rexpro",
								)}
								<a
									class=" text-gray-300 font-medium"
									href="https://github.com/AUTOMATIC1111/stable-diffusion-rexpro/discussions/13993"
									target="_blank"
								>
									{$i18n
										.t(
											"(e.g. `sh rexpro.sh --api --api-auth username_password`)",
										)
										.replace("_", ":")}
								</a>
							</div>
					...
					{:else if config?.IMAGE_GENERATION_ENGINE === "comfyui"}
						...
						<div class="mb-2.5">
							...
							>
								...
								<div class="flex w-full">
									<div
										class="flex-1 mr-2 justify-end flex gap-1"
									>
										{#if config.COMFYUI_WORKFLOW}
											<button
								...
								<div class="mt-1 text-xs flex flex-col gap-1.5">
									{#each REQUIRED_WORKFLOW_NODES as node}
										<div class="flex w-full flex-col">
											...
											<div
												class="flex mt-0.5 items-center"
											>
												<div class="">
													<Tooltip
														content={$i18n.t(
						...
						<div class="mb-2.5">
							...
							>
								...
								<select
									class="w-fit pr-8 cursor-pointer rounded-sm px-2 text-xs bg-transparent outline-hidden text-right"
									bind:value={
										config.IMAGES_GEMINI_ENDPOINT_METHOD
									}
									placeholder={$i18n.t("Select Method")}
								>
									<option value="predict">predict</option>
									<option value="generateContent"
								...
								<div class="flex w-full">
									<div class="flex-1">
										<SensitiveInput
											...
											placeholder={$i18n.t("API Key")}
											bind:value={
												config.IMAGES_EDIT_OPENAI_API_KEY
											}
											required={false}
										/>
									</div>
								...
								<div class="flex w-full">
									<div class="flex-1 mr-2">
										<input
											...
											bind:value={
												config.IMAGES_EDIT_COMFYUI_BASE_URL
											}
										/>
									</div>
									<button
								...
								<div class="flex w-full">
									<div class="flex-1">
										<SensitiveInput
											...
											placeholder={$i18n.t("sk-1234")}
											bind:value={
												config.IMAGES_EDIT_COMFYUI_API_KEY
											}
											required={false}
										/>
									</div>
								...
								<div class="flex w-full">
									<div class="flex-1">
										<SensitiveInput
											...
											placeholder={$i18n.t("API Key")}
											bind:value={
												config.IMAGES_EDIT_GEMINI_API_KEY
											}
											required={true}
										/>
									</div>
