<script lang="ts">
	...
	const cloneHandler = async (func) => {
		const _function = await getFunctionById(
			localStorage.token,
			func.id,
		).catch((error) => {
			toast.error(`${error}`);
		...
		if (res) {
			if (func.is_global) {
				func.type === "filter"
					...
					: toast.success(
							$i18n.t("Function is now globally enabled"),
						);
			} else {
				func.type === "filter"
					? toast.success($i18n.t("Filter is now globally disabled"))
					: toast.success(
							$i18n.t("Function is now globally disabled"),
						);
			}
...
{#if loaded}
				...
				<div class="flex justify-between items-center w-full">
					...
					<div class="flex w-full justify-end gap-1.5">
						{#if $user?.role === "admin"}
							...
							>
								<div
									class=" self-center font-medium line-clamp-1"
								>
									{$i18n.t("Import")}
								</div>
							</button>
								...
								<button
									...
									on:click={async () => {
										...
										if (_functions) {
											...
											saveAs(
												blob,
												`functions-export-${Date.now()}.json`,
											);
										}
									}}
								>
								...
								<div class="flex items-center text-left">
									<div class=" flex-1 self-center pl-1">
										...
										>
											...
											>
												...
												{#if func?.meta?.manifest?.version}
													<div
														class=" text-gray-500 text-xs font-medium shrink-0"
													>
														v{func?.meta?.manifest
															?.version ?? ""}
													</div>
												{/if}
											</div>
										...
										<div class="flex gap-1.5 px-1">
											...
											<div
												class=" text-xs overflow-hidden text-ellipsis line-clamp-1"
											>
							...
							<div class="flex flex-row gap-0.5 self-center">
								...
								{:else}
									...
									<FunctionMenu
										...
										editHandler={() => {
											goto(
												`/admin/functions/edit?id=${encodeURIComponent(func.id)}`,
											);
										}}
										shareHandler={() => {
											shareHandler(func);
		...
		{#if $config?.features.enable_community_sharing}
			<div class=" my-16">
				...
				<a
					...
					href="https://rexpro-ai.rexpro.com/functions"
					target="_blank"
				>
	...
	>
		<div class=" text-sm text-gray-500 truncate">
			...
			<span class="  font-semibold">{selectedFunction.name}</span>.
		</div>
	</DeleteConfirmDialog>
	...
	>
		<div class="text-sm text-gray-500">
			<div
				class=" bg-yellow-500/20 text-yellow-700 dark:text-yellow-200 rounded-lg px-4 py-3"
			>
				<div>
