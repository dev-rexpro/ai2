<script lang="ts">
	...
	import { REXPRO_NAME, config, settings } from "$lib/stores";
...
<Modal bind:show size="xl">
	<div class="px-6 pt-5 dark:text-white text-black">
		...
		<div class="flex items-center mt-1">
			...
			<div
				class="flex self-center w-[1px] h-6 mx-2.5 bg-gray-50/50 dark:bg-gray-850/50"
			/>
			<div class="text-sm dark:text-gray-200">
				v{REXPRO_VERSION}
			</div>
			...
			<div class="mb-3">
				{#if changelog}
					{#each Object.keys(changelog) as version}
						<div class=" mb-3 pr-2">
							<h3
								class="font-semibold text-xl mb-1 dark:text-white m-0"
							>
								v{version} - {changelog[version].date}
							</h3>

							<hr
								class="border-gray-50/50 dark:border-gray-850/50 my-2"
							/>

							{#each Object.keys(changelog[version]).filter((section) => section !== "date") as section}
								<div class="w-full">
									<div
										class="font-semibold uppercase text-xs {section ===
										'added'
									...
									<div
										class="my-2.5 px-1.5 markdown-prose-sm !list-none !w-full !max-w-none"
									>
