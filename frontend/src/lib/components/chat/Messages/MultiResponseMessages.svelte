	...
	const onGroupClick = async (_messageId, modelIdx) => {
		if (messageId != _messageId) {
			...
			let messageChildrenIds =
				history.messages[currentMessageId].childrenIds;
			while (messageChildrenIds.length !== 0) {
	...
	<div>
		...
		>
			{#if $settings?.displayMultiModelResponsesInTabs ?? false}
				<div class="w-full">
					<div
						class=" flex w-full mb-4.5 border-b border-gray-200 dark:border-gray-850"
					>
						<div
						...
						>
							{#each Object.keys(groupedMessageIds) as modelIdx}
								{#if groupedMessageIdsIdx[modelIdx] !== undefined && (groupedMessageIds[modelIdx]?.messageIds ?? []).length > 0}
									...
									<button
										class="min-w-fit {selectedModelIdx ==
										modelIdx
						...
						{#key history.currentId}
							{#if message}
								<ResponseMessage
									...
									showPreviousMessage={() =>
										showPreviousMessage(selectedModelIdx)}
									showNextMessage={() =>
										showNextMessage(selectedModelIdx)}
									{setInputText}
									{updateChat}
									{editMessage}
									{saveMessage}
			...
			{:else}
				{#each Object.keys(groupedMessageIds) as modelIdx}
					{#if groupedMessageIdsIdx[modelIdx] !== undefined && groupedMessageIds[modelIdx].messageIds.length > 0}
						...
						<div
							class=" snap-center w-full max-w-full m-1 border {history
							...
							{#key history.currentId}
								{#if message}
									<ResponseMessage
										...
										{selectedModels}
										isLastMessage={true}
										siblings={groupedMessageIds[modelIdx]
											.messageIds}
										gotoMessage={(message, messageIdx) =>
											gotoMessage(modelIdx, messageIdx)}
										showPreviousMessage={() =>
											showPreviousMessage(modelIdx)}
										showNextMessage={() =>
											showNextMessage(modelIdx)}
										{setInputText}
										{updateChat}
										{editMessage}
										{saveMessage}
				...
				<div class="flex justify-end">
					<div class="w-full">
						{#if history.messages[messageId]?.merged?.status}
							{@const message =
								history.messages[messageId]?.merged}
							...
							<div class="w-full rounded-xl pl-5 pr-2 py-2 mt-2">
								...
								<div
									class="mt-1 markdown-prose w-full min-w-full"
								>
									{#if (message?.content ?? "") === ""}
										<Skeleton />
									{:else}
					...
					{#if isLastMessage}
						<div
							class=" shrink-0 text-gray-600 dark:text-gray-500 mt-1"
						>
							<Tooltip
								content={$i18n.t("Merge Responses")}
								placement="bottom"
							>
								<button
									type="button"
