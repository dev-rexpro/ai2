<script lang="ts">
	...
	const getTextFragmentUrl = (doc: any): string | null => {
		...
		const baseUrl = file_id
			? `${REXPRO_API_BASE_URL}/files/${file_id}/content${page !== undefined ? `#page=${page + 1}` : ""}`
		...
		const words = content
			...
			.filter(
				(w: string) =>
					w.length > 0 &&
					!/https?:\/\/|[\u{1F300}-\u{1F9FF}]/u.test(w),
			);
						...
						>
							<a
								...
								href={document?.metadata?.file_id
									? `${REXPRO_API_BASE_URL}/files/${document?.metadata?.file_id}/content${document?.metadata?.page !== undefined ? `#page=${document.metadata.page + 1}` : ""}`
				...
				{#each mergedDocuments as document, documentIdx}
					<div class="flex flex-col w-full gap-2">
						{#if document.metadata?.parameters}
							<div>
								<div
									class="text-sm font-medium dark:text-gray-300 mb-1"
								>
									{$i18n.t("Parameters")}
								</div>
						...
						<div>
							...
							>
								...
								{#if showRelevance && document.distance !== undefined}
									...
									>
										<div
											class="text-sm my-1 dark:text-gray-400 flex items-center gap-2 w-fit"
										>
											{#if showPercentage}
												{@const percentage =
													calculatePercentage(
							...
							{#if document.metadata?.html}
								...
								></iframe>
							{:else}
								{@const rawContent = document.document
									.trim()
									.replace(/\n\n+/g, "\n\n")}
								{@const isTruncated =
									($settings?.renderMarkdownInPreviews ??
										true) &&
								...
								{:else}
									<pre
										class="text-sm dark:text-gray-400 whitespace-pre-line">{rawContent}</pre>
								{/if}
							{/if}
						</div>
					</div>
