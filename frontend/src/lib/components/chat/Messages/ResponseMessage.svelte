<script lang="ts">
	...
	$: statusEntries = message?.statusHistory ?? [
	...
	const copyToClipboard = async (text) => {
		...
		const res = await _copyToClipboard(
			text,
			null,
	...
	const editMessageConfirmHandler = async () => {
		if (editedOutput) {
			...
			editMessage(
				message.id,
				{ content: message.content, output: editedOutput },
				false,
			);
		} else {
			// Legacy text edit
	...
	const feedbackHandler = async (
		...
		const chat = await getChatById(localStorage.token, chatId).catch(
			(error) => {
				toast.error(`${error}`);
			},
		);
		...
		let feedbackItem = {
			type: "rating",
			data: {
				...(updatedMessage?.annotation
					? updatedMessage.annotation
					: {}),
				model_id: message?.selectedModelId ?? message.model,
				...(history.messages[message.parentId].childrenIds.length > 1
					? {
							sibling_model_ids: history.messages[
								message.parentId
							].childrenIds
								.filter((id) => id !== message.id)
								.map(
									(id) =>
										history.messages[id]?.selectedModelId ??
		...
		if (message?.feedbackId) {
			...
			feedback = await createNewFeedback(
				localStorage.token,
				feedbackItem,
			).catch((error) => {
				toast.error(`${error}`);
		...
		if (!details) {
			...
			if (
				!updatedMessage.annotation?.tags &&
				(message?.content ?? "") !== ""
			) {
	...
	const buttonsWheelHandler = (event: WheelEvent) => {
		if (buttonsContainerElement) {
			if (
				buttonsContainerElement.scrollWidth <=
				buttonsContainerElement.clientWidth
			) {
	...
	onDestroy(() => {
		if (buttonsContainerElement) {
			buttonsContainerElement.removeEventListener(
				"wheel",
				buttonsWheelHandler,
			);
		}
		...
		<div class="flex-auto w-0 pl-1 relative">
			<Name>
				...
				{#if message.timestamp}
					...
					>
						<Tooltip
							content={dayjs(message.timestamp * 1000).format(
								"LLLL",
							)}
						>
			...
			<div>
				...
				>
					<div>
						...
						{#if message?.embeds && message.embeds.length > 0}
							...
							>
								{#each message.embeds as embed, idx}
									<div
										class="my-2 w-full"
										id={`${message.id}-embeds-${idx}`}
									>
										<FullHeightIframe
											src={embed}
											allowScripts={true}
											allowForms={true}
											allowSameOrigin={$settings?.iframeSandboxAllowSameOrigin ??
												false}
											allowPopups={true}
										/>
									</div>
						...
						{#if edit === true}
							<div
								class="w-full bg-gray-50 dark:bg-gray-800 rounded-3xl px-3 py-3 my-2"
							>
								{#if editedOutput}
								...
								<div
									class=" mt-2 mb-1 flex justify-between text-sm font-medium"
								>
									<div>
										<button
						...
						<div
							...
							class="w-full flex flex-col relative {edit
								? 'hidden'
								: ''}"
							...
							{:else if message.content && message.error !== true}
								...
								<ContentRenderer
									...
									floatingButtons={message?.done &&
										!readOnly &&
										($settings?.showFloatingActionButtons ??
											true)}
									save={!readOnly}
									preview={!readOnly}
									{editCodeBlock}
									{topPadding}
									done={($settings?.chatFadeStreamingText ??
									true)
										? (message?.done ?? false)
										: true}
									{model}
									...
									onSourceClick={async (id) => {
										...
										if (citationsElement) {
											citationsElement?.showSourceModal(
												id,
											);
										}
									}}
											...
											>
												{#if action?.icon}
													<div class="size-4">
														<img
															src={action.icon}
															class="w-4 h-4 {action.icon.includes(
																'data:image/svg',
															)
																? 'dark:invert-[80%]'
																: ''}"
