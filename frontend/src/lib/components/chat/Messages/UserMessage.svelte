...
>
	{#if !($settings?.chatBubble ?? true)}
		<div class={`shrink-0 ltr:mr-3 rtl:ml-3 mt-1`}>
			<ProfileImage
				src={user?.id
					? `${REXPRO_API_BASE_URL}/users/${user.id}/profile/image`
					: `${REXPRO_BASE_URL}/static/favicon.png`}
			...
			{#if edit === true}
				...
				>
					{#if (editedFiles ?? []).length > 0}
						<div
							class="flex items-center flex-wrap gap-2 -mx-2 mb-1"
						>
					...
					>
						<textarea
							...
							on:input={(e) => {
								...
								const savedInnerScroll =
									editScrollContainer?.scrollTop;
