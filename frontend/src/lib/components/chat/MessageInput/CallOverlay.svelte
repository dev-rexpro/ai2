<script lang="ts">
	...
	const getVideoInputDevices = async () => {
		...
		if (
			...
			if (
				savedDeviceId &&
				videoInputDevices.some((d) => d.deviceId === savedDeviceId)
			) {
	...
	const transcribeHandler = async (audioBlob) => {
		...
		if (!audioBlob || audioBlob.size < 100) {
			console.log(
				"Audio blob too small or empty, skipping transcription",
			);
			return;
		}
	...
	const monitorAndPlayAudio = async (id, signal) => {
				...
				if (audioCache.has(content)) {
					...
					if ($config.audio.tts.engine !== "") {
						try {
							...
							); // Wait before retrying to reduce tight loop
						} catch (error) {
		...
		console.log(
			`Audio monitoring and playing stopped for message ID ${id}`,
		);
	};
...
{#if $showCallOverlay}
	...
	>
			...
			>
				...
				{:else}
					<div
						...
						style={`background-image: url('${REXPRO_API_BASE_URL}/models/model/profile/image?id=${model?.id}&lang=${$i18n.language}&voice=true');`}
					/>
				{/if}
				<!-- navbar -->
			</button>
		...
		<div
			class="flex justify-center items-center flex-1 h-full w-full max-h-full"
		>
			{#if !camera}
				<button
					type="button"
