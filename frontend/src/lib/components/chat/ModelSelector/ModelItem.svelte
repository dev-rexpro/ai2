...
<button
	...
	aria-label={$i18n.t("Select {{modelName}} model", {
	...
	<div class="flex flex-col flex-1 gap-1.5">
		...
		<div class="flex items-center gap-2">
			<div class="flex items-center min-w-fit">
				...
				>
					<img
						src={`${REXPRO_API_BASE_URL}/models/model/profile/image?id=${item.model.id}&lang=${$i18n.language}`}
						alt={$i18n.t("{{modelName}} profile image", {
							modelName: item.label,
						})}
						class="rounded-md size-5 flex items-center"
						loading="lazy"
						on:error={(e) => {
						...
						<Tooltip elementId="tags-{item.model.id}">
							<div slot="tooltip" id="tags-{item.model.id}">
								{#each item.model?.tags.sort( (a, b) => a.name.localeCompare(b.name), ) as tag}
									...
									>
										<div
											class=" text-xs font-medium rounded-sm uppercase text-white"
										>
											{tag.name}
										</div>
									</Tooltip>
