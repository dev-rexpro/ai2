<script lang="ts">
	...
	const textVariableHandler = async (text: string) => {
		if (text.includes("{{CLIPBOARD}}")) {
			...
			const clipboardItems = await navigator.clipboard
				.read()
				.catch((err) => {
	...
	$: visionCapableModels = (
		...
		(model) =>
			$models.find((m) => m.id === model)?.info?.meta?.capabilities
				?.vision ?? true,
	);
	...
	$: toggleFilters = (
		...
		.map(
			(id) =>
				($models.find((model) => model.id === id) || {})?.filters ?? [],
		)
		.reduce((acc, filters) =>
	...
	const uploadFileHandler = async (file, process = true, itemData = {}) => {
		if (
			$_user?.role !== "admin" &&
			!($_user?.permissions?.chat?.file_upload ?? true)
		) {
		...
		if (!$temporaryChatEnabled) {
			try {
				...
				if (uploadedFile) {
					...
					fileItem.content_type =
						uploadedFile.meta?.content_type ||
			...
			const content = await extractContentFromFile(file).catch(
				(error) => {
					toast.error(
						$i18n.t(
			...
			if (content === null) {
				toast.error(
					$i18n.t("Failed to extract content from the file."),
				);
				files = files.filter((item) => item?.itemId !== tempItemId);
				return null;
			} else {
		...
		inputFiles.forEach(async (file) => {
			...
			if (file["type"].startsWith("image/")) {
				...
				const compressImageHandler = async (
					...
					if (
						configWidth &&
						(width === null || width > configWidth)
					) {
						width = configWidth;
					}
					if (
						configHeight &&
						(height === null || height > configHeight)
					) {
						height = configHeight;
					}
	...
	const createNote = async () => {
		if (
			inputContent?.md.trim() === "" &&
			inputContent?.html.trim() === ""
		) {
	...
	onMount(() => {
		...
		return () => {
			...
			if (dropzoneElement) {
				dropzoneElement.removeEventListener(
					"dragover",
					onDragOver,
					true,
				);
		...
		<div class="bg-transparent">
			...
			>
				<div class="">
					...
					<form
						class="w-full flex flex-col gap-1.5 {recording
							? 'hidden'
							: ''}"
					...
					>
						...
						<div
							...
							class="flex-1 flex flex-col relative w-full gradient-focus-container shadow-sm"
							dir={$settings?.chatDirection ?? "auto"}
						>
							<div
								class="gradient-focus-inner flex-1 flex flex-col w-full px-1 dark:text-gray-100 {$temporaryChatEnabled
									? 'border-dashed border-gray-200 dark:border-gray-800'
									: ''}"
							>
								{#if atSelectedModel !== undefined}
									<div
										class="px-3 pt-3 text-left w-full flex flex-col z-10"
									>
										<div
											class="flex items-center justify-between w-full"
										>
											<div
												class="pl-[1px] flex items-center gap-2 text-sm dark:text-gray-500"
											>
												<img
													alt="model profile"
													class="size-3.5 max-w-[28px] object-cover rounded-full"
													src={`${REXPRO_API_BASE_URL}/models/model/profile/image?id=${$models.find((model) => model.id === atSelectedModel.id).id}&lang=${$i18n.language}`}
												/>
												<div
													class="translate-y-[0.5px]"
												>
													<span class=""
														>{atSelectedModel.name}</span
													>
												</div>
											</div>
											<div>
											...
											{#key $settings?.richTextInput ?? true}
												{#key $settings?.showFormattingToolbar ?? false}
													<RichTextInput
														...
														json={true}
														richText={$settings?.richTextInput ??
															true}
														messageInput={true}
														...
														largeTextAsFile={($settings?.largeTextAsFile ??
															false) &&
															!shiftKey}
														autocomplete={$config
															?.features
															?.enable_autocomplete_generation &&
															($settings?.promptAutocomplete ??
																false)}
														generateAutoCompletion={async (
															text,
														) => {
															if (
								...
								<div
									class=" flex justify-between mt-0.5 mb-2.5 mx-0.5 max-w-full"
									dir="ltr"
								>
									<div
										class="ml-1 self-end flex items-center flex-1 max-w-[80%]"
									>
										<InputMenu
											bind:files
											...
											uploadOneDriveHandler={async (
												...
												try {
													...
													if (fileData) {
														...
														console.log(
															"No file was selected from OneDrive",
														);
													}
												} catch (error) {
													console.error(
										...
										>
											<button
												...
												class="bg-transparent hover:bg-gray-100 text-gray-700 dark:text-white dark:hover:bg-gray-800 rounded-xl size-8 flex justify-center items-center outline-hidden focus:outline-hidden"
												...
												>
													<button
														...
														on:click={() => {
															selectedValvesType =
																"function";
															selectedValvesItemId =
																selectedModelIds[0]?.split(
																	".",
																)[0];
															showValvesModal = true;
														}}
												...
												{#if filter}
													...
													>
														...
														>
															{#if filter?.icon}
																<div
																	class="size-4 items-center flex justify-center"
																>
																	<img
																		src={filter.icon}
																		class="size-3.5 {filter.icon.includes(
																			'data:image/svg',
																		)
																			? 'dark:invert-[80%]'
																			: ''}"
												...
												>
													<button
														on:click|preventDefault={() => {
															...
															const authUrl =
																getOAuthClientAuthorizationUrl(
																	pendingTool.serverId,
																	pendingTool.authType ??
																		"mcp",
																);
															window.open(
																authUrl,
									...
									<div
										class="self-end flex space-x-1 mr-1 shrink-0 gap-[0.5px]"
									>
										{#if isActive && prompt === "" && files.length === 0}
											<div class=" flex items-center">
												...
												>
													<button
														class="bg-white hover:bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-800 transition rounded-xl p-1.5"
														on:click={() => {
															stopResponse();
														}}
													>
										...
										{:else}
											{#if prompt !== "" && !history?.currentId && !$selectedTerminalId && ($config?.features?.enable_notes ?? false) && ($_user?.role === "admin" || ($_user?.permissions?.features?.notes ?? true))}
												...
												>
													<button
														...
														class=" text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 transition rounded-xl p-1.5 -mr-1 self-center"
														type="button"
														disabled={prompt ===
															"" &&
															files.length === 0}
												...
												{#if $_user?.role === "admin" || ($_user?.permissions?.chat?.stt ?? true)}
													...
													>
														<button
															...
															class=" text-gray-600 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-200 transition rounded-xl p-1.5 self-center mr-0.5"
															type="button"
															on:click={async () => {
																try {
																	let stream =
											...
											{#if prompt === "" && files.length === 0 && ($_user?.role === "admin" || ($_user?.permissions?.chat?.call ?? true))}
												<div class=" flex items-center">
													...
													>
														<button
															class=" bg-black text-white hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-xl p-1.5 self-center"
															type="button"
															on:click={async () => {
																if (
						...
						{#if $config?.license_metadata?.input_footer}
							<div
								class=" text-xs text-gray-500 text-center line-clamp-1 marked"
							>
