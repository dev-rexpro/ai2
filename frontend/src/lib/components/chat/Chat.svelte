<script lang="ts">
		...
		if (model) {
			...
			if (model?.info?.meta?.toolIds) {
				...
				selectedToolIds = selectedToolIds.filter(
					(id) => !id.startsWith("direct_server:"),
				);
			}
				...
				if (type === "status") {
					...
					setTimeout(() => {
						const embedEl = document.getElementById(
							`${event.message_id}-embeds-container`,
						);
						if (embedEl) {
							embedEl.scrollIntoView({
								behavior: "smooth",
								block: "center",
							});
						}
	...
	const onMessageHandler = async (event: {
		...
		const isTrusted =
			isSameOrigin || ($settings?.iframeSandboxAllowSameOrigin ?? false);
	...
	onMount(() => {
		...
		const pageSubscribe = page.subscribe(async (p) => {
			if (p.url.pathname === "/") {
				...
				try {
					banners.set(
						await getBanners(localStorage.token).catch(() => []),
					);
				} catch (e) {
		...
		const selectedFolderSubscribe = selectedFolder.subscribe(
			async (folder) => {
				await tick();
				if (
	...
	const uploadWeb = async (urls) => {
		if (
			$user?.role !== "admin" &&
			!($user?.permissions?.chat?.web_upload ?? true)
		) {
			toast.error(
				...
				if (htmlGroups && htmlGroups.length > 0) {
					...
					for (const block of codeBlocks) {
						if (
							block.lang === "svg" ||
							(block.lang === "xml" &&
								block.code.includes("<svg"))
						) {
							contents = [
								...contents,
								{ type: "svg", content: block.code },
							];
						}
					}
				}
	...
	const loadChat = async () => {
		...
		if (chat) {
			...
			if (chatContent) {
				...
				const pendingTaskIds = await getTaskIdsByChatId(
					localStorage.token,
					$chatId,
				)
					.then((res) => res?.task_ids ?? [])
					.catch(() => []);
				const currentMessage = history.currentId
					? history.messages[history.currentId]
					: null;
	...
	const getStopTokens = () => {
		...
		const tokens = Array.isArray(stop)
			? stop
			: stop.split(",").map((s) => s.trim());

		return tokens
			.filter(Boolean)
			.map((token) =>
				decodeURIComponent(
					JSON.parse(`"${token.replace(/"/g, '\\"')}"`),
				),
			);
	};
	...
	const sendMessageSocket = async (
		...
		const terminalEnabled =
			model.info?.meta?.capabilities?.terminal ?? true;
		...
		if (res) {
			if (res.error) {
				...
				const newTaskIds =
					res.task_ids ?? (res.task_id ? [res.task_id] : []);
				if (taskIds) {
	...
	const stopResponse = async (processQueue = true) => {
		if (taskIds) {
			if ($chatId) {
				await stopTasksByChatId(localStorage.token, $chatId).catch(
					(error) => {
						toast.error(`${error}`);
						return null;
					},
			...
			if (
				...
				history.messages[responseMessage.parentId]
			) {
				for (const messageId of history.messages[
					responseMessage.parentId
				].childrenIds) {
...
<svelte:head>
	<title>
		{$settings.showChatTitleInTab !== false && $chatTitle
			? `${$chatTitle.length > 30 ? `${$chatTitle.slice(0, 30)}...` : $chatTitle} • ${$REXPRO_NAME}`
			: `${$REXPRO_NAME}`}
	</title>
</svelte:head>
