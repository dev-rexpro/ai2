<script lang="ts">
	...
	onMount(async () => {
		...
		if (
			...
			(user?.role === "admin" ||
				(user?.permissions?.features?.api_keys ?? false))
		) {
							...
							<div class="flex-1">
								...
								>
									<option value="" selected
										>{$i18n.t("Prefer not to say")}</option
									>
									<option value="male"
										>{$i18n.t("Male")}</option
									>
									<option value="female"
										>{$i18n.t("Female")}</option
									>
									<option value="custom"
										>{$i18n.t("Custom")}</option
									>
								</select>
							</div>
					...
					{#if $user?.role === "admin"}
						<div class="justify-between w-full mt-2">
							<div class="flex justify-between w-full">
								<div
									class="self-center text-xs font-medium mb-1"
								>
									{$i18n.t("JWT Token")}
								</div>
							</div>
