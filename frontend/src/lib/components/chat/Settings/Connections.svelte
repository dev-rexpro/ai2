<script lang="ts">
	...
	const addConnectionHandler = async (connection) => {
		...
		config.OPENAI_API_CONFIGS[config.OPENAI_API_BASE_URLS.length - 1] =
			connection.config;
	...
	const updateHandler = async () => {
		...
		config.OPENAI_API_BASE_URLS = config.OPENAI_API_BASE_URLS.map((url) =>
			url.replace(/\/$/, ""),
		);

		// Check if API KEYS length is same than API URLS length
		if (
			config.OPENAI_API_KEYS.length !== config.OPENAI_API_BASE_URLS.length
		) {
			// if there are more keys than urls, remove the extra keys
			if (
				config.OPENAI_API_KEYS.length >
		...
		{#if config !== null}
			<div class="">
				<div class="pr-1.5">
								...
								<Connection
									...
									onDelete={() => {
										config.OPENAI_API_BASE_URLS =
											config.OPENAI_API_BASE_URLS.filter(
												(url, urlIdx) => idx !== urlIdx,
											);
										config.OPENAI_API_KEYS =
											config.OPENAI_API_KEYS.filter(
												(key, keyIdx) => idx !== keyIdx,
											);

										let newConfig = {};
										config.OPENAI_API_BASE_URLS.forEach(
											(url, newIdx) => {
												newConfig[newIdx] =
					...
					<div class="my-1.5">
						<div
							class="text-xs {($settings?.highContrastMode ??
							false)
						...
						>
							...
							<br />
							{$i18n.t(
								"CORS must be properly configured by the provider to allow requests from rexpro-ai.",
							)}
						</div>
					</div>
				</div>
