<script lang="ts">
	...
	const loadAccessGrants = async () => {
		...
		try {
			accessGrants =
				(await getChatAccessGrants(localStorage.token, chatId)) ?? [];
		} catch (e) {
			...
			<div class="px-5 pt-4 pb-5 w-full flex flex-col">
				...
				{#if chat.share_id}
					<div class="mt-3">
						<AccessControl
							bind:accessGrants
							accessRoles={["read"]}
							sharePublic={$user?.permissions?.sharing
								?.public_chats || $user?.role === "admin"}
				...
				<div class="flex justify-end gap-1 mt-3">
					{#if $config?.features.enable_community_sharing}
						...
						>
							{$i18n.t("Share to rexpro-ai Community")}
						</button>
					{/if}
					...
					<button
						...
						on:click={async () => {
							...
							toast.success(
								$i18n.t("Copied shared chat URL to clipboard!"),
							);
						}}
					>
						<Link />
