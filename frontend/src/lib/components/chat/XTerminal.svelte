<script lang="ts">
	...
	const getTerminalInfo = (): {
		...
		const systemMatch = systemTerminals.find(
			(t: any) => t.id === $selectedTerminalId,
		);
		if (systemMatch) {
			// For system terminals, WS goes through the rexpro-ai backend proxy
			return { serverId: systemMatch.id, baseUrl: REXPRO_API_BASE_URL };
		}

		// Direct terminal (user-configured, matched by URL)
		const directTerminals = ($settings?.terminalServers ?? []).filter(
			(s: any) => s.url,
		);
	...
	const connect = async () => {
		...
		try {
			...
			if (info.serverId === "__direct__") {
				...
				// System terminal — proxy through rexpro-ai backend
				const base = info.baseUrl.replace(/\/$/, "");
				authToken = token;
				...
				const res = await fetch(
					`${base}/terminals/${info.serverId}/api/terminals`,
					{
						method: "POST",
