<script lang="ts">
	...
	import { REXPRO_NAME, config, user } from "$lib/stores";
	...
	const getPromptList = async () => {
		...
		try {
			...
			if (res) {
				...
				// get tags
				tags = await getPromptTags(localStorage.token).catch(
					(error) => {
						toast.error(`${error}`);
						return [];
					},
	...
	const deleteHandler = async (prompt) => {
		...
		const res = await deletePromptById(localStorage.token, prompt.id).catch(
			(err) => {
				toast.error(err);
				return null;
			},
...
{#if loaded}
	...
	>
		<div class=" text-sm text-gray-500 truncate">
			...
			<span class="  font-medium">{deletePrompt.command}</span>.
		</div>
	</DeleteConfirmDialog>
	...
	>
		...
		{#if prompts === null || loading}
			<div
				class="w-full h-full flex justify-center items-center my-16 mb-24"
			>
		...
		{:else if (prompts ?? []).length !== 0}
			...
			<div class="gap-2 grid my-2 px-3 lg:grid-cols-2">
				{#each prompts as prompt (prompt.id)}
					...
					>
						<div
							class=" flex flex-col flex-1 space-x-4 cursor-pointer w-full pl-1"
						>
							<div
								class="flex items-center justify-between w-full mb-0.5"
							>
								<div class="flex items-center gap-2">
									<div
										class="font-medium line-clamp-1 capitalize"
									>
										{prompt.name}
									</div>
									<div
										class="text-xs overflow-hidden text-ellipsis line-clamp-1 text-gray-500"
									>
										/{prompt.command}
									</div>
								</div>
	...
	{#if $config?.features.enable_community_sharing}
		<div class=" my-16">
			...
			<a
				...
				href="https://rexpro-ai.rexpro.com/prompts"
				target="_blank"
			>
