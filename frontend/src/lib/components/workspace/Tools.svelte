<script lang="ts">
	...
	import { REXPRO_NAME, config, tools as _tools, user } from "$lib/stores";
	...
	const shareHandler = async (tool) => {
		const item = await getToolById(localStorage.token, tool.id).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			},
...
{#if loaded}
			...
			<div class="flex w-full justify-end gap-1.5">
				...
				{#if tools.length && ($user?.role === "admin" || $user?.permissions?.workspace?.tools_export)}
					<button
						class="flex text-xs items-center space-x-1 px-3 py-1.5 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-gray-200 transition"
						on:click={async () => {
							const _tools = await exportTools(
								localStorage.token,
							).catch((error) => {
								toast.error(`${error}`);
								return null;
	...
	{#if $config?.features.enable_community_sharing}
		<div class=" my-16">
			...
			<a
				...
				href="https://rexpro-ai.rexpro.com/tools"
				target="_blank"
			>
	...
	>
		<div class=" text-sm text-gray-500 truncate">
			...
			<span class="  font-medium">{selectedTool.name}</span>.
		</div>
	</DeleteConfirmDialog>
