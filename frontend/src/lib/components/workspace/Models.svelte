<script lang="ts">
	...
	const deleteModelHandler = async (model) => {
		const res = await deleteModelById(localStorage.token, model.id).catch(
			(e) => {
				toast.error(`${e}`);
				return null;
			},
	...
	const enableAllHandler = async () => {
		const modelsToEnable = (models ?? []).filter(
			(m) => !(m.is_active ?? true),
		);
	...
	const showAllHandler = async () => {
		const modelsToShow = (models ?? []).filter(
		...
		await Promise.all(
			modelsToShow.map((model) =>
				updateModelById(localStorage.token, model.id, model),
			),
		);
...
{#if loaded}
	...
	>
		...
		{#if models !== null}
			{#if (models ?? []).length !== 0}
				<div
					class=" px-3 my-2 gap-1 lg:gap-2 grid lg:grid-cols-2"
					id="model-list"
				>
					{#each models as model (model.id)}
						...
						>
							<div class="flex group/item gap-3.5 w-full">
								...
								<div
									class=" shrink-0 flex w-full min-w-0 flex-1 pr-1 self-center"
								>
									<div
									...
									>
										<div class="flex-1 w-full">
											...
											<div
												class=" flex gap-1 pr-2 -mt-1 items-center"
											>
												<Tooltip
													content={model?.user
														?.email ??
												...
												>
													<div
														class="shrink-0 text-gray-500 text-xs"
													>
														{$i18n.t(
															"By {{name}}",
															{
												...
												>
													<div
														class="flex gap-1 text-xs overflow-hidden"
													>
														<div
															class="line-clamp-1"
														>
	...
	{#if $config?.features.enable_community_sharing}
		<div class=" my-16">
			...
			<a
				...
				href="https://rexpro-ai.rexpro.com/models"
				target="_blank"
			>
