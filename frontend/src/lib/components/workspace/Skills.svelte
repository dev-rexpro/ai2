<script lang="ts">
	...
	import { REXPRO_NAME, user, skills as _skills } from "$lib/stores";
	...
	const cloneHandler = async (skill) => {
		const _skill = await getSkillById(localStorage.token, skill.id).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			},
	...
	const deleteHandler = async (skill) => {
		const res = await deleteSkillById(localStorage.token, skill.id).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			},
			...
			<div class="flex w-full justify-end gap-1.5">
				...
				{#if total && ($user?.role === "admin" || $user?.permissions?.workspace?.skills)}
					<button
						class="flex text-xs items-center space-x-1 px-3 py-1.5 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-gray-200 transition"
						on:click={async () => {
							const _skills = await exportSkills(
								localStorage.token,
							).catch((error) => {
								toast.error(`${error}`);
								return null;
