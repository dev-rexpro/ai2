<script lang="ts">
	...
	import { REXPRO_NAME, knowledge, user } from "$lib/stores";
	import {
		deleteKnowledgeById,
	...
	const deleteHandler = async (item) => {
		const res = await deleteKnowledgeById(
			localStorage.token,
			item.id,
		).catch((e) => {
			toast.error(`${e}`);
				...
				<div class=" my-2 px-3 grid grid-cols-1 lg:grid-cols-2 gap-2">
					{#each items as item}
						...
						>
							<div class=" w-full">
								<div
									class=" self-center flex-1 justify-between"
								>
									<div
										class="flex items-center justify-between -my-1 h-8"
									>
										<div
									...
									<div
										class=" flex items-center gap-1 justify-between px-1.5"
									>
										<Tooltip
										...
										>
											...
											>
												<div
													class=" text-sm font-medium line-clamp-1 capitalize"
												>
													{item.name}
												</div>
											</div>
										...
										>
											...
											>
												<div
													class=" text-xs text-gray-500 line-clamp-1 hidden sm:block"
												>
													{$i18n.t("Updated")}
													{dayjs(
														item.updated_at * 1000,
