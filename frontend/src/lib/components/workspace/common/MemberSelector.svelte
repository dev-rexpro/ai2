	...
	onMount(async () => {
		...
		if (userIds.length > 0) {
			userIds.forEach(async (id) => {
				const res = await getUserInfoById(localStorage.token, id).catch(
					(error) => {
						console.error(error);
						return null;
					},
							...
							>
								<div>
									...
									<span class="text-xs text-gray-500"
										>{selectedGroup[id].member_count}</span
									>
								</div>
						...
						{#if includeGroups && filteredGroups.length > 0}
							...
							<div class="mb-3">
								{#each filteredGroups as group, groupIdx (group.id)}
									...
									>
										<div
											class="px-3 py-1.5 font-medium text-gray-900 dark:text-white flex-1"
										>
											<div
