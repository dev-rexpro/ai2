			...
			if (url) {
				if (type === "file") {
					if (url.startsWith("http")) {
						...
						window
							.open(
								`${REXPRO_API_BASE_URL}/files/${url}/content`,
								"_blank",
							)
							.focus();
					}
...
>
	...
	{#if !small}
		<div class="flex flex-col justify-center -space-y-0.5 px-2.5 w-full">
			<div
				class=" dark:text-gray-100 text-sm font-medium line-clamp-1 mb-1"
			>
				{decodeString(name)}
			</div>
	...
	{:else}
		...
		>
			<div class="flex flex-col justify-center -space-y-0.5 px-1 w-full">
				<div
					class=" dark:text-gray-100 text-sm flex justify-between items-center"
				>
