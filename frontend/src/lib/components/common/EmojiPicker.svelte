<script lang="ts">
	...
	function saveRecentEmoji(emojiName: string) {
		...
		const updated = [
			emojiName,
			...recentEmojiNames.filter((n) => n !== emojiName),
		].slice(0, MAX_RECENT);
		...
		saveDebounceTimer = setTimeout(async () => {
			await updateUserSettings(localStorage.token, {
				ui: { ...$settings, recentEmojis: updated },
			});
		}, 1000);
	}
			...
			emojis = Object.keys(emojiShortCodes).reduce((acc, key) => {
				if (key.includes(search.toLowerCase())) {
					...
					if (Array.isArray(emojiShortCodes[key])) {
						...
						if (
							emojiShortCodes[key].includes(search.toLowerCase())
						) {
							acc[key] = emojiShortCodes[key];
						}
					}
	...
	$: {
		...
		Object.keys(emojiGroups).forEach((group) => {
			const groupEmojis = emojiGroups[group].filter(
				(emoji) => emojis[emoji],
			);
...
>
	...
	<div slot="content">
		...
		>
			...
			<div
				class="w-full flex justify-start h-96 overflow-y-auto px-3 pb-3 text-sm"
			>
						...
						>
							<div class="w-full mb-2.5">
								{#if item.length === 1 && item[0].type === "group"}
									<!-- Render group header -->
									<div
										class="text-xs font-medium -mb-1 text-gray-500 dark:text-gray-400"
									>
										{item[0].label}
									</div>
								{:else}
