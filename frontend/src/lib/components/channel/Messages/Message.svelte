		...
		{#if swipeOffsetX > 0}
			...
			>
				<div
					...
					class:swipe-reply-icon--active={swipeOffsetX >=
						SWIPE_THRESHOLD}
				>
			...
			class="flex flex-col justify-between w-full max-w-full mx-auto group hover:bg-gray-300/5 dark:hover:bg-gray-700/5 relative {className
				...
				: `px-5 ${
						...
						(message?.reply_to_message?.meta?.model_id ??
							message?.reply_to_message?.user_id) === $user?.id
		...
		>
			...
			{#if message?.is_pinned}
				<div class="flex {showUserProfile ? 'mb-0.5' : 'mt-0.5'}">
					<div
						class="ml-8.5 flex items-center gap-1 px-1 rounded-full text-xs"
					>
						<Pin
							className="size-3 text-yellow-500 dark:text-yellow-300"
						/>
				...
				<div class="relative text-xs mb-1">
					...
					>
						{#if message?.reply_to_message?.meta?.model_id}
							<img
								src={`${REXPRO_API_BASE_URL}/models/model/profile/image?id=${message.reply_to_message.meta.model_id}`}
						...
						{:else}
							<img
								src={message.reply_to_message.user?.role ===
								"webhook"
									? `${REXPRO_API_BASE_URL}/channels/webhooks/${message.reply_to_message.user?.id}/profile/image`
									: `${REXPRO_API_BASE_URL}/users/${message.reply_to_message.user?.id}/profile/image`}
						...
						<div
							class="italic text-sm text-gray-500 dark:text-gray-400 line-clamp-1 w-full flex-1"
						>
							<Markdown
			...
			>
				<div class={`shrink-0 mr-1 w-9`}>
					{#if showUserProfile}
						{#if message?.meta?.model_id}
							<img
								src={`${REXPRO_API_BASE_URL}/models/model/profile/image?id=${message.meta.model_id}`}
								alt={message.meta.model_name ??
						...
						{:else if message.user?.role === "webhook"}
							<ProfileImage
								src={`${REXPRO_API_BASE_URL}/channels/webhooks/${message.user?.id}/profile/image`}
								className={"size-8 ml-0.5"}
							/>
						{:else}
							<ProfilePreview user={message.user}>
								<ProfileImage
									src={`${REXPRO_API_BASE_URL}/users/${message.user?.id}/profile/image`}
									className={"size-8 ml-0.5"}
								/>
							</ProfilePreview>
						{/if}
					{:else}
						...
						{#if message.created_at}
							...
							>
								<Tooltip
									content={dayjs(
										message.created_at / 1000000,
									).format("LLLL")}
								>
									{dayjs(message.created_at / 1000000).format(
										"HH:mm",
									)}
								</Tooltip>
							</div>
				...
				<div class="flex-auto w-0 pl-2">
					{#if showUserProfile}
						<Name>
							<div
								class=" self-end text-base shrink-0 font-medium truncate"
							>
								{#if message?.meta?.model_id}
										...
										<span class="line-clamp-1">
											...
											{:else}
												{$i18n.t(
													...
													{
														LOCALIZED_TIME: dayjs(
															message.created_at /
																1000000,
														).format("LT"),
														LOCALIZED_DATE: dayjs(
															message.created_at /
																1000000,
														).format("L"),
													},
					...
					{#if edit}
						<div class="py-2">
							...
							/>
							<div
								class=" mt-2 mb-1 flex justify-end text-sm font-medium"
							>
					...
					{:else}
						<div
							class=" min-w-full markdown-prose {pending
								? 'opacity-50'
								: ''}"
						>
							...
							{:else}
								...
								/>{#if message.created_at !== message.updated_at && (message?.meta?.model_id ?? null) === null}<span
										class="text-gray-500 text-[10px] pl-1 self-center"
										>({$i18n.t("edited")})</span
									>{/if}
							{/if}
						</div>
						...
						{#if (message?.reactions ?? []).length > 0}
							<div>
								<div
									class="flex items-center flex-wrap gap-y-1.5 gap-1 mt-1 mb-2"
								>
						...
						{#if !thread && message.reply_count > 0}
							<div
								class="flex items-center gap-1.5 -mt-0.5 mb-1.5"
							>
								<button
