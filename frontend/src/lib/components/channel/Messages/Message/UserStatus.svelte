<script lang="ts">
	...
	const directMessageHandler = async () => {
		...
		const res = await getDMChannelByUserId(
			localStorage.token,
			user.id,
		).catch((error) => {
...
{#if user}
	<div class="py-3">
		...
		{#if user?.bio}
			<div class="mx-3.5 mt-2">
				<Tooltip content={user?.bio}>
					<div
						class=" self-center line-clamp-3 flex-1 text-left text-xs"
					>
						{user?.bio}
					</div>
				</Tooltip>
		...
		{#if (user?.groups ?? []).length > 0}
			<div
				class="mx-3.5 mt-2 flex flex-wrap gap-0.5 max-h-20 overflow-y-auto"
			>
