	...
	<div>
		...
		{:else if !thread}
			<div class="px-5 max-w-full mx-auto">
				...
				{:else}
					<div
						class="flex justify-center text-xs items-center gap-2 py-5"
					>
				...
				{#if messageList.length > 0}
					<hr
						class=" border-gray-50 dark:border-gray-700/20 py-2.5 w-full"
					/>
				{/if}
			</div>
		{/if}
		...
		{#each messageList as message, messageIdx (id ? `${id}-${message.id}` : message.id)}
			<Message
				...
				showUserProfile={messageIdx === 0 ||
					messageList.at(messageIdx - 1)?.user_id !==
						message.user_id ||
					messageList.at(messageIdx - 1)?.user?.id !==
						message.user?.id ||
					messageList.at(messageIdx - 1)?.meta?.model_id !==
