...
<PinnedMessagesModal
	bind:show={showChannelPinnedMessagesModal}
	{channel}
	{onPin}
/>
<ChannelInfoModal bind:show={showChannelInfoModal} {channel} {onUpdate} />
<nav
	class="sticky top-0 z-30 w-full px-1.5 py-1 -mb-8 flex items-center drag-region flex flex-col"
>
	<div
			...
			>
				{#if channel}
					<div class="flex items-center gap-0.5 shrink-0">
						{#if channel?.type === "dm"}
							{#if channel?.users}
								{@const channelMembers = channel.users.filter(
									(u) => u.id !== $user?.id,
								)}
								<div class="flex mr-1.5 relative">
									{#each channelMembers.slice(0, 2) as u, index}
										<img
											src={`${REXPRO_API_BASE_URL}/users/${u.id}/profile/image`}
											alt={u.name}
							...
							{:else}
								<Users
									className="size-4 ml-1 mr-0.5"
									strokeWidth="2"
								/>
							{/if}
						{:else}
							<div
								class=" size-4.5 justify-center flex items-center"
							>
						...
						<div
							class=" text-left self-center overflow-hidden w-full line-clamp-1 flex-1"
						>
							{#if channel?.name}
								{channel.name}
							{:else}
			...
			>
				{#if channel}
					<Tooltip content={$i18n.t("Pinned Messages")}>
						...
						>
							<div
								class=" flex items-center gap-0.5 m-auto self-center shrink-0"
							>
