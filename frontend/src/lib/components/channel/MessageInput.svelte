<script lang="ts">
	...
	let inputVariablesModalCallback: (
				...
				const compressImageHandler = async (
					...
					const settingsCompression =
						(settings?.imageCompression &&
							settings?.imageCompressionInChannels) ??
						false;
					const configWidth =
			...
			>
				<div class="flex flex-col px-3 w-full">
					...
					{#if typingUsers.length > 0}
						...
						>
							<div
								class=" text-xs px-1 mt-1.5 flex items-center gap-1.5"
							>
								<Skeleton size="xs" />

								<div>
									<span
										class=" font-normal text-black dark:text-white"
									>
										{typingUsers
											.map((user) => user.name)
											.join(", ")}
						...
						>
							{#if replyToMessage !== null}
								...
								>
									...
									>
										<div
											class="pl-[1px] flex items-center gap-2 text-sm"
										>
							...
							{#if files.length > 0}
								<div
									class="mx-2 mt-2.5 -mb-1 flex flex-wrap gap-2"
								>
									...
									{#key $settings?.richTextInput && $settings?.showFormattingToolbar}
										<RichTextInput
											...
											on:keydown={async (e) => {
												e = e.detail.event;
												const isCtrlPressed =
													e.ctrlKey || e.metaKey; // metaKey is for Cmd key on Mac
							...
							<div class=" flex justify-between mb-2.5 mx-0.5">
								<div
									class="ml-1 self-end flex space-x-1 flex-1"
								>
									<slot name="menu">
										{#if acceptFiles}
											<InputMenu
