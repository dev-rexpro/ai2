<script lang="ts">
	...
	const initHandler = async () => {
		...
		channel = await getChannelById(localStorage.token, id).catch(
			(error) => {
				return null;
			},
		);
	...
	const submitHandler = async ({ content, data }) => {
		...
		const res = await sendMessage(localStorage.token, id, message).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			},
...
<svelte:head>
	{#if channel?.type === "dm"}
		...
		>
	{:else}
		<title>#{channel?.name ?? "Channel"} • rexpro-ai</title>
	{/if}
</svelte:head>
			...
			{#if channel && messages !== null}
				<div class="flex-1 overflow-y-auto">
					<div
						...
						on:scroll={(e) => {
							scrollEnd =
								Math.abs(messagesContainerElement.scrollTop) <=
								50;
						}}
					>
						{#key id}
