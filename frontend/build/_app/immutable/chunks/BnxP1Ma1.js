import "./DKem_M_z.js";
import "./DwLkIEu4.js";
import { p as push, i as getContext, l as legacy_pre_effect, w as set, k as get, m as deep_read_state, n as legacy_pre_effect_reset, c as child, r as reset, z as sibling, t as template_effect, u as untrack, d as set_text, y as event, a as append, b as pop, e as store_get, s as setup_stores, q as mutable_source, h as first_child, aR as next, aW as text, x as derived_safe_equal, f as from_html, B as from_svg, aS as createEventDispatcher, aV as mutate, A as tick } from "./B-Kw_l3V.js";
...
import { c as config, u as user, m as models } from "./BJ3rUC-5.js";
import { t as toast } from "./B4Bn1xMI.js";
import { a as getChatById, s as shareChatById, b as getChatAccessGrants, e as deleteSharedChatById, u as updateChatAccessGrants } from "./DwAGGfMc.js";
import { m as copyToClipboard } from "./X0cYWxFO.js";
...
import { K as Knowledge } from "./D_mSRSNF.js";
import { g as getFolderById } from "./Bta4KKyK.js";
var root$3 = from_html(`<a target="_blank"> <span class=" underline"> </span>.</a> <button class="underline"> </button> `, 1);
var root_1$2 = from_html(`<div class="mt-3"><!></div>`);
var root_2$1 = from_html(`<button class="flex items-center gap-1 px-3.5 py-2 text-sm font-medium bg-gray-100 hover:bg-gray-200 text-gray-800 dark:bg-gray-850 dark:text-white dark:hover:bg-gray-800 transition rounded-full" type="button"> </button>`);
var root_3$1 = from_html(`<div class="px-5 pt-4 pb-5 w-full flex flex-col"><div class="text-sm dark:text-gray-300"><!></div> <!> <div class="flex justify-end gap-1 mt-3"><!> <button class="flex items-center gap-1 px-3.5 py-2 text-sm font-medium bg-black hover:bg-gray-900 text-white dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-full" type="button" id="copy-and-share-chat-button"><!> <!></button></div></div>`);
var root_4 = from_html(`<div><div class=" flex justify-between dark:text-gray-300 px-5 pt-4 pb-0.5"><div class=" text-lg font-medium self-center"> </div> <button class="self-center"><!></button></div> <!></div>`);
function ShareChatModal($$anchor, $$props) {
  ...
  const loadAccessGrants = async () => {
    if (!chatId()) return;
    try {
      set(accessGrants, await getChatAccessGrants(localStorage.token, chatId()) ?? []);
    } catch (e) {
  ...
  const saveAccessGrants = async () => {
    try {
      await updateChatAccessGrants(localStorage.token, chatId(), get(accessGrants));
    ...
    children: ($$anchor2, $$slotProps) => {
      ...
      var node_1 = sibling(div_1, 2);
      {
        var consequent_4 = ($$anchor3) => {
          ...
          var node_3 = sibling(div_4, 2);
          {
            var consequent_1 = ($$anchor4) => {
              ...
              var node_4 = child(div_5);
              {
                let $0 = derived_safe_equal(() => ($user(), untrack(() => {
                  var _a, _b, _c, _d;
                  return ((_c = (_b = (_a = $user()) == null ? void 0 : _a.permissions) == null ? void 0 : _b.sharing) == null ? void 0 : _c.public_chats) || ((_d = $user()) == null ? void 0 : _d.role) === "admin";
                })));
                AccessControl(node_4, {
                  accessRoles: ["read"],
                  get sharePublic() {
                    return get($0);
                  },
            ...
            if_block(node_3, ($$render) => {
              if (get(chat), untrack(() => get(chat).share_id)) $$render(consequent_1);
            });
          }
          ...
          var node_5 = child(div_6);
          {
            var consequent_2 = ($$anchor4) => {
              ...
              template_effect(($0) => set_text(text_7, $0), [
                () => ($i18n(), untrack(() => $i18n().t("Share to rexpro-ai Community")))
              ]);
          ...
          var node_7 = sibling(node_6, 2);
          {
            ...
            if_block(node_7, ($$render) => {
              if (get(chat), untrack(() => get(chat).share_id)) $$render(consequent_3);
          ...
          event("click", button_3, async () => {
            ...
            if (isSafari) {
              /* @__PURE__ */ console.log("isSafari");
              const getUrlPromise = async () => {
                const url = await shareLocalChat();
                return new Blob([url], { type: "text/plain" });
              };
              navigator.clipboard.write([new ClipboardItem({ "text/plain": getUrlPromise() })]).then(() => {
                /* @__PURE__ */ console.log("Async: Copying to clipboard was successful!");
                return true;
              }).catch((error) => {
                /* @__PURE__ */ console.error("Async: Could not copy text: ", error);
                return false;
              });
            } else {
        ...
        if_block(node_1, ($$render) => {
          if (get(chat)) $$render(consequent_4);
        });
      }
      reset(div);
      template_effect(
        ...
        [
          () => ($i18n(), untrack(() => $i18n().t("Share Chat"))),
...
function FolderMenu($$anchor, $$props) {
  ...
  let onCreateSubFolder = prop($$props, "onCreateSubFolder", 8, () => {
  });
    ...
    $$slots: {
      ...
      content: ($$anchor2, $$slotProps) => {
        ...
        template_effect(
          ...
          [
            () => ($i18n(), untrack(() => $i18n().t("Create Folder"))),
...
function FolderModal($$anchor, $$props) {
  ...
  const submitHandler = async () => {
    ...
    const maxFileCount = ((_c = (_b = $config()) == null ? void 0 : _b.features) == null ? void 0 : _c.folder_max_file_count) ?? "";
    if (maxFileCount && (((_d = get(data)) == null ? void 0 : _d.files) ?? []).length > maxFileCount) {
      toast.error($i18n().t("Maximum number of files per folder is {{max}}.", { max: maxFileCount ?? 0 }));
      set(loading, false);
      return;
    }
    await onSubmit()({
      ...
      meta: get(meta),
      data: get(data),
      parent_id: edit() ? void 0 : parentId()
    });
    show(false);
    set(loading, false);
  };
    ...
    children: ($$anchor2, $$slotProps) => {
      ...
      event("change", input_2, (e) => {
        ...
        if (inputFiles && inputFiles.length > 0 && ["image/gif", "image/webp", "image/jpeg", "image/png"].includes(inputFiles[0]["type"])) {
          ...
          /* @__PURE__ */ console.log(`Unsupported File Type '${inputFiles[0]["type"]}'.`);
          e.target.value = "";
        }
      });
      event("click", button_1, () => {
