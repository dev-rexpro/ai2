...
import { e as PluginKey, f as Plugin, S as Slice, F as Fragment, T as TextSelection, A as AllSelection, N as NodeSelection, h as Node, D as DecorationSet, i as Decoration, E as Extension, k as keymap } from "./Bv1TNWY0.js";
import { A as tick } from "./B-Kw_l3V.js";
...
const getVariable = (name) => isNode ? undefinedToNull(define_process_env_default[name.toUpperCase().replaceAll("-", "_")]) : undefinedToNull(varStorage.getItem(name));
const hasConf = (name) => hasParam("--" + name) || getVariable(name) !== null;
const production = hasConf("production");
const forceColor = isNode && isOneOf(define_process_env_default.FORCE_COLOR, ["true", "1", "2"]);
...
$constructedBy($Never);
const _$Object = class _$Object extends Schema {
  /**
   * @param {S} shape
...
const print = (...args) => {
  /* @__PURE__ */ console.log(...computeLoggingArgs(args));
...
const removeEventHandlerListener = (eventHandler, f) => {
  ...
  if (len === eventHandler.l.length) {
    /* @__PURE__ */ console.error("[yjs] Tried to remove event handler that doesn't exist.");
  }
};
...
class ContentDoc {
  ...
  constructor(doc2) {
    if (doc2._item) {
      /* @__PURE__ */ console.error("This document was already integrated as a sub-document. You should create a second instance instead with the same guid.");
    }
    this.doc = doc2;
    const opts = {};
    this.opts = opts;
...
if (glo[importIdentifier] === true) {
  /* @__PURE__ */ console.error("Yjs was already imported. This breaks constructor checks and will lead to issues! - https://github.com/yjs/yjs/issues/438");
}
...
const defaultDeleteFilter = (item, protectedNodes) => !(item instanceof Item) || !(item.content instanceof ContentType) || !(item.content.type instanceof YText || item.content.type instanceof YXmlElement && protectedNodes.has(item.content.type.nodeName)) || item.content.type._length === 0;
...
class SocketIOCollaborationProvider {
  constructor(documentId, socket, user, initialContent = null) {
    __publicField(this, "doc", new Doc());
    __publicField(this, "awareness", new SimpleAwareness(this.doc));
    __publicField(this, "isConnected", false);
    __publicField(this, "synced", false);
    __publicField(this, "editor", null);
    __publicField(this, "editorContentGetter", null);
  ...
  setEditor(editor, editorContentGetter) {
    this.editor = editor;
  ...
  joinDocument() {
    ...
    this.socket.emit("ydoc:document:join", {
      ...
      user_id: (_a = this.user) == null ? void 0 : _a.id,
      user_name: (_b = this.user) == null ? void 0 : _b.name,
        ...
        try {
          if (data.state) {
            ...
            if (state.length === 2 && state[0] === 0 && state[1] === 0) {
              const isEmptyEditor = !((_a = this.editor) == null ? void 0 : _a.getText().trim());
              if (isEmptyEditor && this.editor) {
                if (this.initialContent && ((data == null ? void 0 : data.sessions) ?? [""]).length === 1) {
                  if (typeof this.initialContent === "string") {
                    ...
                    if (editorYdoc) {
                      applyUpdate(this.doc, encodeStateAsUpdate(editorYdoc));
                    }
                  }
                }
              } else {
                if (this.doc.getXmlFragment("prosemirror").length > 0) {
                  this.socket.emit("ydoc:document:update", {
                    document_id: this.documentId,
                    user_id: (_b = this.user) == null ? void 0 : _b.id,
              ...
              applyUpdate(this.doc, state, "server");
            }
          }
          this.synced = true;
        } catch (error) {
    ...
    this.doc.on("update", async (update, origin) => {
      ...
      if (this.editor && origin !== "server" && this.isConnected) {
        ...
        this.socket.emit("ydoc:document:update", {
          ...
          data: {
            content: ((_b = this.editorContentGetter) == null ? void 0 : _b.call(this)) ?? {
              md: "",
              html: "",
              json: ""
            }
...
class SimpleAwareness {
  constructor(doc2) {
    ...
    __publicField(this, "_updateHandlers");
    __publicField(this, "_localState");
    this.doc = doc2;
    this.clientID = doc2.clientID ? doc2.clientID : Math.floor(Math.random() * 4294967295);
  ...
  encodeUpdate(clients) {
    const obj = {};
    for (const id2 of clients || Array.from(this._states.keys())) {
      const st = this._states.get(id2);
      if (st) obj[id2] = st;
    }
