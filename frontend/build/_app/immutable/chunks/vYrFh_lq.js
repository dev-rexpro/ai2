...
import { b0 as writable, o as onMount$1, b3 as state, k as get$1, w as set$1, A as tick, bT as svelte, bU as settled } from "./B-Kw_l3V.js";
const version = "7fbf7e7ac2747b6d0081827a66a30d02b77efd7a";
class HttpError {
  /**
   * @param {number} status
...
function parse_route_id(id) {
  const params = [];
  const pattern = id === "/" || root_group_pattern.test(id) ? /^\/$/ : new RegExp(
...
const base = ((_a = globalThis.__sveltekit_n8s9kz) == null ? void 0 : _a.base) ?? "";
const assets = ((_b = globalThis.__sveltekit_n8s9kz) == null ? void 0 : _b.assets) ?? base ?? "";
...
if (is_legacy) {
  ...
  page = new (_c = class {
    constructor() {
      ...
      __privateAdd(this, _url, state(new URL(placeholder_url)));
    }
    get data() {
...
function discard_load_cache() {
  var _a2;
  void ((_a2 = load_cache == null ? void 0 : load_cache.fork) == null ? void 0 : _a2.then((f) => f == null ? void 0 : f.discard()));
  load_cache = null;
...
async function start(_app, _target, hydrate) {
  var _a2, _b2, _c2, _d2;
  if (globalThis.__sveltekit_n8s9kz.data) {
    const { q = {}, p = {}, l = {}, f = {} } = globalThis.__sveltekit_n8s9kz.data;
    for (const k in q) q[k];
    for (const k in l) l[k];
    for (const k in f) f[k];
    for (const k in p) p[k];
  }
      ...
      if (options.invalidateAll) {
        ...
        for (const [id, entries] of query_map) {
          for (const [payload, entry] of entries) {
            (_a2 = entry.resource) == null ? void 0 : _a2.reset();
    ...
    void tick().then(tick).then(() => {
      for (const [id, entries] of query_map) {
        for (const [payload, { resource }] of entries) {
          if (query_keys == null ? void 0 : query_keys.has(create_remote_key(id, payload))) {
            void resource.start();
          }
        }
      }
      for (const [id, entries] of live_query_map) {
        for (const [payload, { resource }] of entries) {
          if (live_query_keys == null ? void 0 : live_query_keys.has(create_remote_key(id, payload))) {
  ...
  if (errors && false) {
    ...
    result.props.errors = await Promise.all(
      ...
      branch.map((b, i) => {
        ...
        last_idx = i;
        return (_a2 = errors[i]) == null ? void 0 : _a2.call(errors).then((e) => e.component).catch(() => void 0);
      }).filter((e) => e !== null)
    );
  }
  if (error && false) {
...
async function load_route({ id, invalidating, url, params, route, preload }) {
  ...
  errors.forEach((loader) => loader == null ? void 0 : loader().catch(noop));
  loaders.forEach((loader) => loader == null ? void 0 : loader[1]().catch(noop));
...
async function load_root_error_page({ status, error, url, route }) {
  ...
  try {
    ...
    if (error2 instanceof Redirect) {
      await _goto(new URL(error2.location, location.href), {}, 0);
      return;
    }
    const error_template = await app.get_error_template();
    const handled = await handle_error(error2, { url, params, route });
    const message = String((handled == null ? void 0 : handled.message) ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
async function navigate({
  ...
  const nav = type === "enter" ? create_navigation(current, intent, url, type) : _before_navigate({
    ...
    intent,
    scroll: popped == null ? void 0 : popped.scroll,
    // @ts-ignore
    event
  });
  if (!nav) {
  ...
  const load_cache_fork = intent && (load_cache == null ? void 0 : load_cache.id) === intent.id ? load_cache.fork : null;
  if ((load_cache == null ? void 0 : load_cache.fork) && !load_cache_fork) {
    discard_load_cache();
  } else {
    load_cache = null;
    current_a = { element: void 0, href: void 0 };
  ...
  if (started) {
    ...
    if (!keepfocus && document.activeElement instanceof HTMLElement && document.activeElement !== document.body) {
    ...
    if (fork) {
      ...
      commit_promise = (_a2 = settled) == null ? void 0 : _a2();
    }
    has_navigated = true;
  } else {
...
function setup_preload() {
  ...
  async function preload(element, priority) {
    ...
    const interacted = a === current_a.element && (a == null ? void 0 : a.href) === current_a.href && priority >= current_priority;
...
async function _hydrate(target2, { status = 200, error, node_ids, params, route, server_route, data: server_data_nodes, form }) {
  ...
  try {
    ...
    result = await get_navigation_result_from_branch({
      ...
      status,
      error,
      errors: parsed_route == null ? void 0 : parsed_route.errors,
      // TODO load earlier?
      form,
    ...
    if (error2 instanceof Redirect) {
      return await native_navigation(new URL(error2.location, location.href));
    }
